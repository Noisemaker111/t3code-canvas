...
> {
  return Effect.gen(function* () {
    ...
    // Prefer an explicit loopback from HOST / --dev-url / --host when given.
    // On Windows, default to 127.0.0.1: `localhost` often resolves to ::1 while
    // the server binds 127.0.0.1, so WebSocket never connects and a plain
    // `bun run` / `npm run` board load hangs forever.
    const webLoopbackHost = (() => {
      if (isDesktopMode) {
        return DESKTOP_DEV_LOOPBACK_HOST;
      }
      for (const candidate of [host, baseEnv.HOST?.trim(), devUrl?.hostname]) {
        if (candidate === undefined || candidate === "") continue;
        // Bind-all addresses are not usable as browser/WS hostnames.
        if (candidate === "0.0.0.0" || candidate === "::") continue;
        if (candidate === "127.0.0.1" || candidate === "::1" || candidate === "localhost") {
          return candidate;
        }
      }
      return platform === "win32" ? DESKTOP_DEV_LOOPBACK_HOST : "localhost";
    })();

    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      PORT: String(webPort),
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://${webLoopbackHost}:${webPort}`,
    };
    ...
    if (!isDesktopMode) {
      ...
      output.VITE_HTTP_URL = `http://${webLoopbackHost}:${serverPort}`;
      output.VITE_WS_URL = `ws://${webLoopbackHost}:${serverPort}`;
    } else {
      output.T3CODE_PORT = String(serverPort);
    ...
    if (isDesktopMode) {
      ...
      // Vite reads HOST; without this it binds ::1-only and 127.0.0.1 breaks.
      output.HOST = webLoopbackHost;
    }
