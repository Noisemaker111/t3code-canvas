...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("pins web loopback urls to 127.0.0.1 when HOST asks for it", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
    ...
    it.effect("defaults Windows web loopback urls to 127.0.0.1 so plain bun/npm run works", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
