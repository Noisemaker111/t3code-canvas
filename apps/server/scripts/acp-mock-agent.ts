...
import { deriveMockReply } from "./acpMockReply.ts";
...
/** Answer prompts from their own declared response shape instead of a fixed string. */
const deriveReplies = process.env.T3_ACP_REPLY_MODE === "derive";
...
/**
 * Subcommands the Cursor/Grok providers probe before they will speak ACP.
 * Handling them here means a settings `binaryPath` can point straight at this
 * file — no wrapper script — and the provider reports installed + signed in.
 */
function runProbeSubcommand(argv: ReadonlyArray<string>): boolean {
  const command = argv.find((arg) => !arg.startsWith("-"));
  if (command === "about") {
    const version = process.env.T3_ACP_MOCK_VERSION ?? "2099.01.01-mock";
    ...
    process.stdout.write(
      [`CLI Version         ${version}`, "Logged In           mock@localhost", ""].join("\n"),
    );
    return true;
  }
  if (argv.includes("--version") || argv.includes("-v")) {
    process.stdout.write(`${process.env.T3_ACP_MOCK_VERSION ?? "2099.01.01-mock"}\n`);
    return true;
  }
  return false;
}

if (runProbeSubcommand(process.argv.slice(2))) {
  process.exit(0);
}
      ...
      yield* agent.client.sessionUpdate({
        ...
        update: {
          ...
          content: {
            ...
            text:
              promptResponseText ??
              (deriveReplies ? deriveMockReply(promptTextFromRequest(request)) : "hello from mock"),
          },
        },
      });
