  ...
  (config) =>
    Effect.gen(function* () {
      ...
      if (yield* fs.exists(path.join(webDist, "index.html"))) {
        ...
        // A server package without its client starts successfully but every
        // browser request returns 503. Fail the release build here instead of
        // producing an artifact that cannot be downloaded and launched.
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(webDist, "index.html"),
        });
      }
    }),
