...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("ships configurable thread navigation defaults", () =>
    Effect.sync(() => {
      ...
      // No mounted handler in any build ships these as defaults any more;
      // the command ids stay valid for stored user bindings.
      assert.equal(defaultsByCommand.get("thread.previous"), undefined);
      assert.equal(defaultsByCommand.get("thread.next"), undefined);
      assert.equal(defaultsByCommand.get("thread.jump.1"), undefined);
      assert.equal(defaultsByCommand.get("sidebar.toggle"), undefined);
  ...
  it.effect("fails when config directory is not writable", () =>
    Effect.gen(function* () {
      // A 0o000 directory does not block root, so the failure this test needs
      // never happens. T3J runs as root by design, so this is a normal
