...
/**
 * Routing failures used to read as a bare "failed to resolve the VCS driver" —
 * no path, no reason — which is unreadable in a board tick log that only ever
 * shows the message. Upstream detail stays in the cause chain (a detection
 * failure can carry git's own output); the shape of the failure does not, and
 * it is the part that tells the reader what to do.
 */
function routeDetail(headline: string, cwd: string, cause: unknown): string {
  const because =
    cause instanceof VcsUnsupportedOperationError
      ? " No git repository was found there."
      : " Detecting a repository there failed.";
  return `${headline} (${cwd}).${because}`;
}
      ...
      Effect.mapError(
        (cause) =>
          new GitManagerError({
            ...
            cwd,
            detail: routeDetail(
              "Failed to resolve the VCS driver for this Git workflow",
              cwd,
              cause,
            ),
            cause,
