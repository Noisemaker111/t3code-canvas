...
describe("GitWorkflowService", () => {
  ...
  it.effect("names the path when routing a workflow to a directory that is not a repo", () => {
    // Never reached: the route fails first. Built eagerly by `Effect.andThen`.
    const runStackedAction = () => Effect.never;
    const testLayer = GitWorkflowService.layer.pipe(
      Layer.provide(
        Layer.mock(VcsDriverRegistry.VcsDriverRegistry)({
          detect: () => Effect.succeed(null),
      ...
      Layer.provide(Layer.mock(GitVcsDriver.GitVcsDriver)({})),
      Layer.provide(Layer.mock(GitManager.GitManager)({ runStackedAction })),
    );
    ...
    return Effect.gen(function* () {
      ...
      const error = yield* workflow
        .runStackedAction({
          actionId: "a1",
          cwd: "/root/worktrees/reaped",
          action: "commit_push_pr",
        })
        .pipe(Effect.flip);

      // The tick log only ever shows the message, so both halves have to be in it.
      expect(error.message).toContain("/root/worktrees/reaped");
      expect(error.message).toContain("No git repository was found there.");
    }).pipe(Effect.provide(testLayer));
  });
