...
const riftFixtureBin = NodeURL.fileURLToPath(
  new URL("../vcs/__fixtures__/rift-bin", import.meta.url),
);
...
function createTextGeneration(
  ...
  return {
    ...
    assistPrompt: (input) => Effect.succeed({ operation: input.operation, text: input.text }),
  };
}
  ...
  return {
    service: {
      ...
      mergePullRequest: (input) =>
        execute({
          cwd: input.cwd,
          args: ["pr", "merge", input.reference, "--squash", "--delete-branch"],
        }).pipe(Effect.as(null)),
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  // The board commits after the coding agent is already done, so a failed model
  // call here used to throw the finished work away with the reaped worktree.
  it.effect("commits with a plain subject when commit-message generation fails", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      NodeFS.mkdirSync(NodePath.join(repoDir, "apps", "server"), { recursive: true });
      NodeFS.writeFileSync(NodePath.join(repoDir, "apps", "server", "a.ts"), "a\n");
      NodeFS.writeFileSync(NodePath.join(repoDir, "apps", "server", "b.ts"), "b\n");
      ...
      ).toBe("Update apps/server (2 files)");
    }),
  );
  ...
  it.effect("featureBranch keeps the current branch when its commits are already made", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      yield* runGit(repoDir, ["push", "-u", "origin", "main"]);
      yield* runGit(repoDir, ["checkout", "-b", "feature/agent-work"]);
      NodeFS.writeFileSync(NodePath.join(repoDir, "agent.txt"), "agent\n");
      yield* runGit(repoDir, ["add", "agent.txt"]);
      yield* runGit(repoDir, ["commit", "-m", "Agent commit"]);
      ...
      ).toBe("feature/agent-work");
    }),
  );

  it.effect("featureBranch moves already-made commits off the default branch", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      yield* runGit(repoDir, ["push", "-u", "origin", "main"]);
      const pushedMain = yield* runGit(repoDir, ["rev-parse", "main"]).pipe(
        Effect.map((r) => r.stdout.trim()),
      );
      NodeFS.writeFileSync(NodePath.join(repoDir, "agent.txt"), "agent\n");
      yield* runGit(repoDir, ["add", "agent.txt"]);
      yield* runGit(repoDir, ["commit", "-m", "Agent commit on main"]);
      const workSha = yield* runGit(repoDir, ["rev-parse", "HEAD"]).pipe(
        Effect.map((r) => r.stdout.trim()),
      );
      ...
      expect(result.branch.status).toBe("created");
      expect(result.branch.name).toBe("feature/agent-commit-on-main");
      expect(result.commit.status).toBe("skipped_no_changes");
      expect(result.push.status).toBe("pushed");
      expect(
        yield* runGit(repoDir, ["rev-parse", "feature/agent-commit-on-main"]).pipe(
          Effect.map((r) => r.stdout.trim()),
        ),
      ).toBe(workSha);
      // The commits are proposed once, from the feature branch only.
      expect(
        yield* runGit(repoDir, ["rev-parse", "main"]).pipe(Effect.map((r) => r.stdout.trim())),
      ).toBe(pushedMain);
    }),
  );
  ...
  it.effect("create_pr opens the PR off the commit log when content generation fails", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      yield* runGit(repoDir, ["checkout", "-b", "feature/pr-content-fallback"]);
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      NodeFS.writeFileSync(NodePath.join(repoDir, "fallback.txt"), "fallback\n");
      yield* runGit(repoDir, ["add", "fallback.txt"]);
      yield* runGit(repoDir, ["commit", "-m", "Teach the board to ship"]);
      ...
      expect(result.pr.status).toBe("created");
      expect(result.pr.title).toBe("Teach the board to ship");
