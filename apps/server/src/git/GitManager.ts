...
import { resolveCommitIdentity } from "./commitIdentity.ts";
...
const missionTokens = (value: string): ReadonlySet<string> =>
  new Set(
    value
      .toLowerCase()
      .split(/[^a-z0-9]+/g)
...
/**
 * A generated subject that shares no word with the task it was generated for is
 * the signature of text written from a diff that is not the card's. It is not
 * proof, so it warns rather than refusing — the refusals live in the pipeline.
 */
function warnWhenOffMission(input: {
  readonly kind: "commit" | "pullRequest";
  readonly cwd: string;
  readonly subject: string;
  readonly mission: string | undefined;
}): Effect.Effect<void> {
  if (!input.mission || input.mission.trim().length === 0) return Effect.void;
  const missionWords = missionTokens(input.mission);
  if (missionWords.size === 0) return Effect.void;
  const subjectWords = missionTokens(input.subject);
  for (const word of subjectWords) {
    if (missionWords.has(word)) return Effect.void;
  }
  return Effect.logWarning(`generated ${input.kind} subject shares no word with the task`, {
    cwd: input.cwd,
...
/**
 * What to commit as when the model cannot say.
 *
 * Commit and PR text are generated by a model call, and that call fails for
 * reasons that have nothing to do with the change: no key, rate limit, provider
 * 5xx. For the board's git pipeline that call runs *after* the coding thread is
 * finished, so a failure there used to throw the whole task away — work sitting
 * uncommitted in a worktree the next launch reaps. A plain subject off the file
 * list is worse prose and infinitely better than that.
 */
function fallbackCommitSubject(stagedSummary: string): string {
  const paths = stagedSummary
    .split(/\r?\n/g)
    .map((line) => line.split(/\t/)[1]?.trim() ?? "")
    .filter((path) => path.length > 0);
  if (paths.length === 0) return "Update project files";
  const first = paths[0] ?? "";
  const area = first.split("/").slice(0, 2).join("/") || first;
  const subject = paths.length === 1 ? `Update ${first}` : `Update ${area} (${paths.length} files)`;
  return subject.slice(0, 72).trimEnd();
}

/** First commit subject in `git log --oneline base..HEAD`, sha stripped. */
function firstCommitSubject(commitSummary: string): string | null {
  const line = commitSummary.split(/\r?\n/g).find((entry) => entry.trim().length > 0);
  const subject = line?.replace(/^[0-9a-f]{7,40}\s+/i, "").trim() ?? "";
...
export const make = Effect.gen(function* () {
  ...
  const resolveCommitAndBranchSuggestion = Effect.fn("resolveCommitAndBranchSuggestion")(
    function* (input: {
      ...
      /** What the work was launched to do, when the caller knows. */
      ...
      const generated = yield* textGeneration
        ...
        .pipe(
          ...
          Effect.catch((cause) =>
            Effect.logWarning("commit message generation failed; committing with a plain subject", {
              cwd: input.cwd,
              detail: cause instanceof Error ? cause.message : String(cause),
            }).pipe(
              Effect.as(
                sanitizeCommitMessage({
                  subject: fallbackCommitSubject(context.stagedSummary),
                  body: "",
                }),
              ),
            ),
  ...
  const runCommitStep = Effect.fn("runCommitStep")(function* (
    ...
    const identity = yield* serverSettingsService.getSettings.pipe(
      Effect.map(resolveCommitIdentity),
      Effect.mapError(
        (cause) =>
          new GitManagerError({
            operation: "runCommitStep",
            cwd,
            detail: "Failed to read the commit identity from server settings.",
            cause,
          }),
      ),
    );
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const generated = yield* textGeneration
      ...
      .pipe(
        Effect.catch((cause) =>
          Effect.logWarning(`${terms.singular} content generation failed; using the commit log`, {
            cwd,
            detail: cause instanceof Error ? cause.message : String(cause),
          }).pipe(
            Effect.as({
              title: firstCommitSubject(rangeContext.commitSummary) ?? branch,
              body: limitContext(rangeContext.commitSummary.trim(), 20_000),
            }),
          ),
        ),
      );

    yield* warnWhenOffMission({ kind: "pullRequest", cwd, subject: generated.title, mission });
  ...
  const preparePullRequestThread: GitManager["Service"]["preparePullRequestThread"] = Effect.fn(
    ...
    return yield* Effect.gen(function* () {
      ...
      // Scoped like every other thread workspace — `<root>/<projectSlug>/thread-<id>` —
      // instead of the branch-named flat default: a flat PR worktree matches
      // no project's own directory, so `isThreadWorktreePath` never sees it as
      // something it owns and the reaper leaves it on disk forever. Falls back
      // to the PR reference when no thread owns this worktree yet (nothing is
      // in Active for it), which is still unique and still under the project's
      // own root.
      const worktree = yield* gitCore.createWorktree({
        ...
        path: buildThreadWorktreePath({
          ...
          projectCwd: input.cwd,
          threadId: input.threadId ?? `pr-${sanitizeWorktreeSegment(normalizedReference)}`,
        }),
      });
  ...
  const readHeadCommitSubject = Effect.fn("readHeadCommitSubject")(function* (cwd: string) {
    const result = yield* gitCore.execute({
      operation: "GitManager.runFeatureBranchStep.headSubject",
      cwd,
      args: ["log", "-1", "--pretty=%s"],
  ...
  /**
   * Leave the default branch where its remote is once its unpushed commits have
   * a feature branch of their own. Without it the same commits are still ahead
   * on the next run and get proposed a second time under a new branch.
   */
  const rewindDefaultBranchToUpstream = Effect.fn("rewindDefaultBranchToUpstream")(function* (
    cwd: string,
    defaultBranch: string,
    upstreamRef: string,
  ) {
    const contains = yield* gitCore.execute({
      operation: "GitManager.runFeatureBranchStep.upstreamIsAncestor",
      cwd,
    ...
    yield* gitCore.execute({
      operation: "GitManager.runFeatureBranchStep.rewindDefaultBranch",
      cwd,
      args: ["branch", "--force", defaultBranch, upstreamRef],
      allowNonZeroExit: true,
    });
  });

  /**
   * A feature-branch run over a clean worktree. The board asks for a feature
   * branch on every pull request, so failing here strands work an agent already
   * committed itself — the commits exist, only the branch to propose them from
   * is missing.
   */
  const resolveFeatureBranchForCommittedWork = Effect.fn("resolveFeatureBranchForCommittedWork")(
    function* (cwd: string) {
      const status = yield* gitCore.statusDetails(cwd);

      if (status.branch && !status.isDefaultBranch && status.aheadOfDefaultCount > 0) {
        return { status: "skipped_not_requested" as const };
      }

      if (status.branch && status.isDefaultBranch && status.aheadCount > 0) {
        const subject = yield* readHeadCommitSubject(cwd);
        const existingBranchNames = yield* gitCore.listLocalBranchNames(cwd);
        const resolvedBranch = resolveAutoFeatureBranchName(
          existingBranchNames,
          sanitizeFeatureBranchName(subject),
        );
        ...
        if (status.upstreamRef) {
          yield* rewindDefaultBranchToUpstream(cwd, status.branch, status.upstreamRef);
        }
        return { status: "created" as const, name: resolvedBranch };
      }
      ...
      return yield* new GitManagerError({
        ...
        cwd,
        detail:
          "Cannot create a feature branch because there are no changes to commit and " +
          "no unpushed commits to propose.",
      });
    },
  );
  ...
  const runFeatureBranchStep = Effect.fn("runFeatureBranchStep")(function* (
    ...
    if (!suggestion) {
      return {
        branchStep: yield* resolveFeatureBranchForCommittedWork(cwd),
        ...
        const pr = wantsPr
          ? yield* progress
              ...
              .pipe(
                ...
                Effect.flatMap(() =>
                  runPrStep(modelSelection, input.cwd, currentBranch, progress.emit, input.mission),
                ),
              )
          : { status: "skipped_not_requested" as const };
