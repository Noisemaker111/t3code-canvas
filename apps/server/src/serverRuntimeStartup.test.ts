...
it.effect("settleStaleSessionsAtStartup interrupts sessions left running by a dead process", () =>
  Effect.gen(function* () {
    const staleRunningThreadId = ThreadId.make("thread-stale-running");
    const staleStartingThreadId = ThreadId.make("thread-stale-starting");
    const settledThreadId = ThreadId.make("thread-settled");
    const threadShellBase = {
      projectId: ProjectId.make("project-1"),
      title: "Thread",
      modelSelection: ServerRuntimeStartup.getAutoBootstrapDefaultModelSelection(),
    ...
    yield* ServerRuntimeStartup.settleStaleSessionsAtStartup.pipe(
      Effect.provideService(ProjectionSnapshotQuery.ProjectionSnapshotQuery, {
        getCommandReadModel: () => Effect.die("unused"),
        getSnapshot: () => Effect.die("unused"),
        getShellSnapshot: () =>
          Effect.succeed({
            ...
            threads: [
              {
                ...threadShellBase,
                id: staleRunningThreadId,
                session: {
                  ...sessionBase,
                  threadId: staleRunningThreadId,
                  status: "running",
                  activeTurnId: "turn-1",
                },
              },
              {
                ...threadShellBase,
                id: staleStartingThreadId,
                session: {
                  ...sessionBase,
                  threadId: staleStartingThreadId,
                  status: "starting",
                  activeTurnId: null,
                },
              },
              ...
              { ...threadShellBase, id: ThreadId.make("thread-no-session"), session: null },
            ],
          } as never),
        getArchivedShellSnapshot: () => Effect.die("unused"),
        getSnapshotSequence: () => Effect.die("unused"),
      ...
      Effect.provideService(OrchestrationEngine.OrchestrationEngineService, {
        readEvents: () => Stream.empty,
        dispatch: (command) =>
          Ref.update(dispatched, (calls) => [...calls, command as Record<string, unknown>]).pipe(
            Effect.as({ sequence: 1 }),
          ),
        streamDomainEvents: Stream.empty,
        latestSequence: Effect.succeed(0),
    ...
    for (const command of commands) {
      ...
      const session = command.session as Record<string, unknown>;
      assert.equal(session.status, "interrupted");
      assert.equal(session.activeTurnId, null);
    }
    assert.deepStrictEqual(
      commands.map((command) => command.threadId),
      [staleRunningThreadId, staleStartingThreadId],
    );
  }),
);
