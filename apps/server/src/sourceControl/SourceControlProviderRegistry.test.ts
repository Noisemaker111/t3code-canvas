...
it.effect("keeps GitHub check reads when repository context is bound", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@github.com:Noisemaker111/vps-code.git" }],
      github: {
    ...
    const provider = yield* registry.resolve({ cwd: "/repo" });
    assert.notEqual(provider.getChangeRequestChecks, undefined);
    const checks = yield* provider.getChangeRequestChecks!({ cwd: "/repo", reference: "#249" });
    assert.strictEqual(checks.state, "passing");
    assert.strictEqual(checks.counts.passing, 1);
  }),
);
