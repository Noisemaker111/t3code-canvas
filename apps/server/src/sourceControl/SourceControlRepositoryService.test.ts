...
it.effect("adopts an existing checkout whose origin matches the requested remote", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const parent = yield* fs.makeTempDirectoryScoped({ prefix: "t3-source-control-adopt-" });
    const destinationPath = `${parent}/t3code`;
    yield* fs.makeDirectory(destinationPath, { recursive: true });
    // Make the destination non-empty so it can never be cloned into.
    yield* fs.writeFileString(`${destinationPath}/README.md`, "# existing checkout\n");
    const cloneCalls: Array<ReadonlyArray<string>> = [];

    yield* Effect.gen(function* () {
      const service = yield* SourceControlRepositoryService.SourceControlRepositoryService;
      // protocol "auto" resolves to the SSH url; the on-disk origin is the HTTPS
      // url for the same repo — adoption must treat them as equivalent.
      const result = yield* service.cloneRepository({
        provider: "github",
        repository: "octocat/t3code",
        destinationPath,
      ...
      // Adoption must NOT invoke `git clone`.
      assert.deepStrictEqual(cloneCalls, []);
    }).pipe(
      Effect.provide(
        makeLayer({
            ...
            execute: (input) =>
              Effect.sync(() => {
                if (input.args[0] === "remote") {
                  return { ...processOutput(), stdout: `${CLONE_URLS.url}\n` };
                }
...
it.effect("refuses with an actionable error when the destination is a different repository", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const parent = yield* fs.makeTempDirectoryScoped({ prefix: "t3-source-control-mismatch-" });
    const destinationPath = `${parent}/t3code`;
    yield* fs.makeDirectory(destinationPath, { recursive: true });
    yield* fs.writeFileString(`${destinationPath}/README.md`, "# other repo\n");
    ...
    yield* Effect.gen(function* () {
      ...
      assert.strictEqual(error.operation, "cloneRepository");
      assert.strictEqual(error.provider, "github");
      assert.include(error.detail, destinationPath);
      assert.include(error.detail, "different git repository");
      assert.include(error.detail, "https://github.com/other/thing");
      assert.include(error.detail, "Add that folder as an existing project");
    }).pipe(
      Effect.provide(
        makeLayer({
          git: {
            execute: (input) =>
              input.args[0] === "remote"
                ? Effect.succeed({
                    ...processOutput(),
                    stdout: "https://github.com/other/thing\n",
                  })
...
it.effect("refuses with an actionable error when the destination is not a git repo", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const parent = yield* fs.makeTempDirectoryScoped({ prefix: "t3-source-control-nonrepo-" });
    const destinationPath = `${parent}/t3code`;
    yield* fs.makeDirectory(destinationPath, { recursive: true });
    yield* fs.writeFileString(`${destinationPath}/notes.txt`, "just some files\n");
    ...
    yield* Effect.gen(function* () {
      ...
      assert.strictEqual(error.operation, "cloneRepository");
      assert.include(error.detail, destinationPath);
      assert.include(error.detail, "is not a git checkout of");
      assert.include(error.detail, "Add that folder as an existing project");
    }).pipe(
      Effect.provide(
        makeLayer({
          git: {
            // `git remote get-url origin` exits non-zero: not a repository.
            execute: (input) =>
              input.args[0] === "remote"
                ? Effect.succeed({
                    ...processOutput(),
