import {
  type SourceControlAccountIdentity,
...
import {
  GITHUB_ACCOUNT_IDENTITY_ARGS,
  parseGitHubAccountIdentity,
} from "./gitHubAccountIdentity.ts";
...
export class SourceControlDiscovery extends Context.Service<
  SourceControlDiscovery,
  {
    ...
    readonly accountIdentity: Effect.Effect<SourceControlAccountIdentity | null>;
  }
...
export const make = Effect.gen(function* () {
  ...
  const accountIdentity = process
    .run({
      operation: "source-control.discovery.account-identity",
      command: "gh",
      args: GITHUB_ACCOUNT_IDENTITY_ARGS,
      cwd: config.cwd,
    ...
    .pipe(
      Effect.map((result) =>
        result.exitCode === 0 ? parseGitHubAccountIdentity(result.stdout) : null,
      ),
