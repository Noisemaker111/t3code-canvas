...
function bindProviderContext(
  ...
  return SourceControlProvider.SourceControlProvider.of({
    ...
    // Optional capability: forward it only when the underlying provider has
    // one, so `mergeChangeRequest === undefined` still means "this forge
    // cannot merge from the board".
    ...(provider.mergeChangeRequest
      ? {
          ...
          }) => provider.mergeChangeRequest!({ ...input, context: input.context ?? context }),
        }
      : {}),
    ...(provider.getChangeRequestChecks
      ? {
          getChangeRequestChecks: (input: {
            ...
            readonly context?: SourceControlProvider.SourceControlProviderContext;
            readonly reference: string;
          }) => provider.getChangeRequestChecks!({ ...input, context: input.context ?? context }),
        }
      : {}),
  });
}
