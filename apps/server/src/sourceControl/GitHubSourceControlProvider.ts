...
export const make = Effect.gen(function* () {
  ...
  return SourceControlProvider.SourceControlProvider.of({
    ...
    closeChangeRequest: (input) =>
      github.closePullRequest({ cwd: input.cwd, reference: input.reference }).pipe(
        Effect.mapError(
          (error) =>
            new SourceControlProviderError({
              provider: "github",
              operation: "closeChangeRequest",
              command: error.command,
              cwd: input.cwd,
              reference: SourceControlProvider.transportSafeSourceControlErrorValue(
                input.reference,
    ...
    getChangeRequestChecks: (input) =>
      github.getPullRequestChecks({ cwd: input.cwd, reference: input.reference }).pipe(
        Effect.map(parseStatusCheckRollup),
        Effect.mapError(
          (error) =>
            new SourceControlProviderError({
              provider: "github",
