...
export function annotateEnvironmentRequest(endpoint: string) {
  return Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer((exit) => {
      ...
      const errorTag = causeErrorTag(exit.cause);
      // A missing resource (e.g. a thread the client hasn't caught up on
      // deletion for yet) is an expected, routine outcome the client already
      // handles gracefully — logging it as a warning on every request just
      // spams the server console without signaling anything actionable.
      if (errorTag === "EnvironmentResourceNotFoundError") {
        return Effect.logDebug("environment api request failed", {
          endpoint,
          traceId,
          errorTag,
          cause: exit.cause,
      ...
      return Effect.logWarning("environment api request failed", {
        endpoint,
        traceId,
        errorTag,
        cause: exit.cause,
