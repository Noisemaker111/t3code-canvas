...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("uses T3CODE_STATIC_DIR for production web assets", () =>
    Effect.gen(function* () {
      const staticDir = " /opt/t3j/current/apps/server/dist/client ";
      const resolved = yield* resolveServerConfig(
        {
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(ConfigProvider.fromEnv({ env: { T3CODE_STATIC_DIR: staticDir } })),
            NetService.layer,
          ),
        ),
      );

      expect(resolved.staticDir).toBe(staticDir.trim());
    }),
  );
