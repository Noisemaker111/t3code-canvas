...
it.effect("resolves a durable board service token for Hermes Agent", () =>
  Effect.gen(function* () {
    const registry = yield* McpSessionRegistry.__testing
      .make({
        now: () => 1_000,
        boardServiceToken: "hermes-board-token-test",
      })
      .pipe(
    ...
    const resolved = yield* registry.resolve("hermes-board-token-test");
    expect(resolved?.providerInstanceId).toBe(
      McpSessionRegistry.BOARD_SERVICE_PROVIDER_INSTANCE_ID,
    );
    expect(resolved?.threadId).toBe(McpSessionRegistry.BOARD_SERVICE_THREAD_ID);
    expect(resolved?.capabilities.has("kanban")).toBe(true);
