...
export interface McpSessionRegistryOptions {
  ...
  readonly now?: () => number;
  /**
   * Durable board-MCP bearer for an external Hermes Agent (gateway/cron).
   * Defaults to `T3CODE_BOARD_MCP_TOKEN`. Not a provider-session token — kanban only.
   */
  readonly boardServiceToken?: string;
}

/** Synthetic scope for the always-on Hermes Agent board loop. */
export const BOARD_SERVICE_THREAD_ID = ThreadId.make("thread_hermes_board");
export const BOARD_SERVICE_PROVIDER_INSTANCE_ID = ProviderInstanceId.make("hermes");
export const BOARD_SERVICE_PROVIDER_SESSION_ID = "hermes-board-service";
...
const makeWithOptions = Effect.fn("McpSessionRegistry.make")(function* (
  ...
  const boardServiceToken = (
    options.boardServiceToken ??
    process.env.T3CODE_BOARD_MCP_TOKEN ??
    ""
  ).trim();
  const endpoint =
  ...
  const issue: McpSessionRegistryShape["issue"] = Effect.fn("McpSessionRegistry.issue")(
    function* (request) {
      ...
      const scope: McpInvocationContext.McpInvocationScope = {
        ...
        // All issued provider MCP sessions get board + preview toolkits.
        capabilities: new Set(["preview", "kanban"]),
        issuedAt,
        expiresAt,
      };
  ...
  const resolve: McpSessionRegistryShape["resolve"] = Effect.fn("McpSessionRegistry.resolve")(
    function* (rawToken) {
      ...
      // Durable Hermes Agent board token — not stored in the session map, never expires
      // with idle prune. Kanban-only (no preview automation).
      if (boardServiceToken.length > 0 && timingSafeTokenEqual(rawToken, boardServiceToken)) {
        const timestamp = yield* currentTimeMillis;
        const scope: McpInvocationContext.McpInvocationScope = {
          environmentId,
          threadId: BOARD_SERVICE_THREAD_ID,
          providerSessionId: BOARD_SERVICE_PROVIDER_SESSION_ID,
          providerInstanceId: BOARD_SERVICE_PROVIDER_INSTANCE_ID,
          capabilities: new Set(["kanban"]),
          issuedAt: timestamp,
          // Far future so capability checks that compare expiresAt stay valid.
          expiresAt: timestamp + 10 * 365 * 24 * 60 * 60 * 1_000,
        };
        return scope;
      }
      const tokenHash = yield* hashToken(rawToken);
