...
it("falls back to a stable spawn directory when the server's own cwd is gone", () => {
  // Install rotates release directories away under the running process, and
  // once its cwd is deleted `process.cwd()` throws on every call.
  const stable = GitVcsDriver.gitSpawnDir(() => {
    throw Object.assign(new Error("ENOENT: uv_cwd"), { code: "ENOENT" });
  });
  assert.isTrue(stable.length > 0);
  assert.strictEqual(
    GitVcsDriver.gitSpawnDir(() => "/opt/t3j/releases/abc"),
    "/opt/t3j/releases/abc",
  );
});
