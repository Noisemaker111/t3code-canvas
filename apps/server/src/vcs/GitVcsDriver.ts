...
/** Author and committer to stamp on a commit, in place of the box's git config. */
...
export interface GitCommitOptions {
  ...
  readonly identity?: GitCommitIdentity | null;
}
...
/**
 * Where the probe spawns git from — deliberately never the directory it is
 * asking about, so `-C <target>` lets git answer for a path that is gone
 * instead of the spawn failing first.
 *
 * `process.cwd()` throws once the server's own working directory is deleted,
 * and this box deletes it routinely: Install writes a new release directory and
 * rotates old ones away under the running process. That throw escapes as a
 * defect from every git probe at once, which reads as the whole board losing
 * git rather than as one stale path.
 */
export function gitSpawnDir(readCwd: () => string = () => globalThis.process.cwd()): string {
  try {
    return readCwd();
  } catch {
