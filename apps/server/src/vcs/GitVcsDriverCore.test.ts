...
const riftFixtureBin = NodeURL.fileURLToPath(new URL("./__fixtures__/rift-bin", import.meta.url));
process.env.PATH = `${riftFixtureBin}:${process.env.PATH ?? ""}`;
...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("structured errors", () => {
    ...
    it.effect("does not wrap a remove-worktree command failure in a synthetic error", () =>
      Effect.gen(function* () {
        ...
        // The actionable reason Rift prints to stderr must reach the error detail
        // instead of being collapsed into a bare "non-zero status" message.
        assert.include(error.detail, "workspace does not exist");
      }),
    );
  });
  ...
  describe("repository status", () => {
    ...
    it.effect("re-anchors a HEAD that points at a missing object", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        ...
        const goodHead = yield* git(cwd, ["rev-parse", "HEAD"]);
        yield* writeTextFile(cwd, `.git/refs/heads/${initialBranch}`, `${"0".repeat(40)}\n`);

        const status = yield* (yield* GitVcsDriver.GitVcsDriver).statusDetails(cwd);

        assert.equal(status.isRepo, true);
        assert.equal(status.branch, initialBranch);
        assert.equal(yield* git(cwd, ["rev-parse", "HEAD"]), goodHead);
      }),
    );

    it.effect("says why a corrupt HEAD could not be re-anchored", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const pathService = yield* Path.Path;
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        yield* writeTextFile(cwd, `.git/refs/heads/${initialBranch}`, `${"0".repeat(40)}\n`);
        yield* fileSystem.remove(pathService.join(cwd, ".git", "logs"), { recursive: true });

        const failure = yield* Effect.flip((yield* GitVcsDriver.GitVcsDriver).statusDetails(cwd));

        assert.instanceOf(failure, GitCommandError);
        assert.include(failure.detail, "Could not repair HEAD");
      }),
    );
  ...
  describe("worktree operations", () => {
    it.effect("creates and removes a worktree for a new refName", () =>
      Effect.gen(function* () {
        ...
        const driver = yield* GitVcsDriver.GitVcsDriver;
        const remote = yield* makeTmpDir("git-rift-origin-");
        yield* git(remote, ["init", "--bare"]);
        yield* git(cwd, ["remote", "add", "origin", remote]);
        yield* git(cwd, ["push", "origin", "main"]);
    ...
    it.effect("heals a source checkout whose HEAD is corrupt before snapshotting it", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const pathService = yield* Path.Path;
        ...
        // Both the source and the workspace an agent gets handed resolve HEAD.
        assert.equal(yield* git(cwd, ["rev-parse", "HEAD"]), goodHead);
        assert.equal(yield* git(created.worktree.path, ["rev-parse", "HEAD"]), goodHead);
      }),
    );

    it.effect("falls back to a plain git worktree when Rift init cannot snapshot the source", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        // Make the fake `rift init` fail exactly like a source directory that
        // lives on a filesystem without copy-on-write reflink support.
        yield* writeTextFile(cwd, ".rift-fake-init-fail", "");
        ...
        // The workspace is created at the requested path via `git worktree add`,
        // on the requested branch — no raw reflink error surfaces.
        assert.equal(created.worktree.path, worktreePath);
        assert.equal(created.worktree.refName, "feature/worktree");
        assert.equal(yield* git(worktreePath, ["branch", "--show-current"]), "feature/worktree");
        // It is a genuine linked git worktree of the source repository.
        assert.equal(
          yield* git(worktreePath, ["rev-parse", "HEAD"]),
          yield* git(cwd, ["rev-parse", "HEAD"]),
        );
        // The merge base for the new branch is configured just like the Rift path.
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.feature/worktree.gh-merge-base"),
          "main",
        );
      }),
    );
    ...
    it.effect(
      "fallback refuses a pre-existing non-empty worktree target with an actionable error",
      () =>
        Effect.gen(function* () {
          const cwd = yield* makeTmpDir();
          const { initialBranch } = yield* initRepoWithCommit(cwd);
          ...
          // The target already exists and is not empty: `git worktree add` would
          // fail with a raw "already exists" — we want the actionable message.
          yield* writeTextFile(worktreePath, "occupied.txt", "in the way\n");
          ...
          assert.equal(error.operation, "GitVcsDriver.createWorktree.gitWorktreeAdd");
          assert.include(error.detail, worktreePath);
          assert.include(error.detail, "already exists and is not empty");
        }),
    );

    it.effect("re-raises a non-reflink Rift init failure unchanged", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        // A generic (non-reflink) rift init failure must NOT trigger the git
        // worktree fallback — it stays a hard error with Rift's own reason.
        yield* writeTextFile(cwd, ".rift-fake-init-generic-fail", "");
        ...
        assert.deepInclude(error, {
          _tag: "GitCommandError",
          operation: "GitVcsDriver.createWorktree.initRift",
          command: "rift",
        });
      }),
    );

    it.effect("clears a stale Rift marker and retries init before creating a workspace", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        // Plant the same orphaned-marker failure mode the VPS hits when
        // `.rift` outlives the local registry entry.
        yield* writeTextFile(cwd, ".rift", "STALE-RIFT-MARKER\n");
        ...
        const created = yield* driver.createWorktree({
          ...
          path: worktreePath,
          refName: initialBranch,
          newRefName: "feature/stale-marker",
        });

        assert.equal(created.worktree.path, worktreePath);
        assert.equal(created.worktree.refName, "feature/stale-marker");
        assert.equal(yield* fileSystem.exists(pathService.join(cwd, ".rift")), false);
        assert.equal(
          yield* git(worktreePath, ["branch", "--show-current"]),
          "feature/stale-marker",
        );
      }),
    );

    it.effect("always invokes rift with an absolute --into, even with no requested path", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        const pathService = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const argsLog = pathService.join(yield* makeTmpDir("rift-args-"), "argv.txt");
        const worktreeRoot = yield* makeTmpDir("git-worktree-root-");
        process.env.RIFT_FAKE_ARGS_LOG = argsLog;
        process.env.T3CODE_WORKTREE_ROOT = worktreeRoot;

        const created = yield* driver
          .createWorktree({
            cwd,
            // No requested path — this is what every non-test caller passes,
            // and what used to leave rift resolving the workspace against `/`.
            path: null,
            refName: initialBranch,
            newRefName: "feature/worktree",
          })
          .pipe(
            Effect.ensuring(
              Effect.sync(() => {
                delete process.env.RIFT_FAKE_ARGS_LOG;
                delete process.env.T3CODE_WORKTREE_ROOT;
              }),
            ),
          );

        const argv = yield* fileSystem.readFileString(argsLog);
        const intoMatch = /--into (\S+)/.exec(argv);
        assert.isNotNull(intoMatch, `rift create was invoked without --into: ${argv}`);
        assert.isTrue(
          pathService.isAbsolute(intoMatch?.[1] ?? ""),
          `--into was not absolute: ${argv}`,
        );
        assert.equal(intoMatch?.[1], worktreeRoot);
        assert.equal(pathService.dirname(created.worktree.path), worktreeRoot);
      }),
    );

    it.effect("fallback refuses to overwrite a colliding worktree path instead of forcing", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        const worktreePath = pathService.join(worktreeRoot, "thread-one");
        // First worktree wins the path.
        yield* driver.createWorktree({
          cwd,
          path: worktreePath,
          refName: initialBranch,
          newRefName: "feature/first",
        });

        // A second claim on the same directory is a bug, not something to
        // bulldoze: `git worktree add --force` used to overwrite it silently.
        const error = yield* driver
          .createWorktree({
            cwd,
            path: worktreePath,
            refName: initialBranch,
            newRefName: "feature/second",
          })
          .pipe(Effect.flip);

        assert.include(error.detail ?? "", worktreePath);
        // The first worktree is untouched.
        assert.equal(yield* git(worktreePath, ["branch", "--show-current"]), "feature/first");
      }),
    );

    it.effect("keeps per-thread isolation on the plain git worktree fallback", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        yield* git(cwd, ["remote", "add", "origin", remote]);
        yield* git(cwd, ["push", "origin", "main"]);
        // ext4-style box: no reflink support, so every worktree takes the
        // fallback. Isolation must survive that, since it is the common path.
        yield* writeTextFile(cwd, ".rift-fake-init-fail", "");
        ...
        const first = yield* driver.createWorktree({
          cwd,
          path: pathService.join(worktreeRoot, "thread-aaaa"),
          refName: initialBranch,
          newRefName: "t3/worktree/aaaa",
          baseRefName: "origin/main",
        });
        const second = yield* driver.createWorktree({
          cwd,
          path: pathService.join(worktreeRoot, "thread-bbbb"),
          refName: initialBranch,
          newRefName: "t3/worktree/bbbb",
          baseRefName: "origin/main",
        });

        assert.notEqual(first.worktree.path, second.worktree.path);
        assert.equal(
          yield* git(first.worktree.path, ["branch", "--show-current"]),
          "t3/worktree/aaaa",
        );
        assert.equal(
        ...
        // And teardown works on the fallback too (rift remove cannot see these).
        yield* driver.removeWorktree({ cwd, path: first.worktree.path });
        const fileSystem = yield* FileSystem.FileSystem;
        assert.equal(yield* fileSystem.exists(first.worktree.path), false);
  ...
  describe("commit context", () => {
    ...
    it.effect("stamps the configured identity instead of the box's git config", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        yield* writeTextFile(cwd, "a.txt", "a\n");
        yield* driver.prepareCommitContext(cwd, ["a.txt"]);
        yield* driver.commit(cwd, "Add a", "", {
          identity: { name: "NoisemakerJon", email: "jon@users.noreply.github.com" },
        });

        assert.equal(
          yield* git(cwd, ["log", "-1", "--pretty=%an <%ae>|%cn <%ce>"]),
          "NoisemakerJon <jon@users.noreply.github.com>|NoisemakerJon <jon@users.noreply.github.com>",
        );
      }),
    );

    it.effect("says why staging failed", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;

        yield* writeTextFile(cwd, "fix.txt", "fix\n");
        yield* writeTextFile(cwd, ".git/index.lock", "");

        const failure = yield* Effect.flip(driver.prepareCommitContext(cwd));

        assert.instanceOf(failure, GitCommandError);
        assert.include(failure.detail, "index.lock");
      }),
    );
  ...
  describe("remote operations", () => {
    ...
    it.effect("publishes a unique remote branch when the requested name has diverged", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        const remote = yield* makeTmpDir("git-remote-");
        ...
        yield* git(other, ["config", "user.email", "other@test.com"]);
        yield* git(other, ["config", "user.name", "Other"]);
        yield* git(other, ["checkout", "-b", "feature/collision"]);
        yield* writeTextFile(other, "remote.txt", "remote\n");
        yield* git(other, ["add", "."]);
        yield* git(other, ["commit", "-m", "Remote work"]);
        yield* git(other, ["push", "origin", "feature/collision"]);

        yield* driver.createRef({ cwd, refName: "feature/collision" });
        yield* driver.switchRef({ cwd, refName: "feature/collision" });
        yield* writeTextFile(cwd, "local.txt", "local\n");
        yield* driver.prepareCommitContext(cwd);
        yield* driver.commit(cwd, "Local work", "");
        ...
        assert.deepInclude(pushed, {
          ...
          upstreamBranch: "origin/feature/collision-2",
          setUpstream: true,
        });
        assert.equal(
          yield* git(cwd, ["rev-parse", "--abbrev-ref", "@{upstream}"]),
          "origin/feature/collision-2",
        );
        assert.equal(
          yield* git(remote, ["log", "-1", "--pretty=%s", "feature/collision"]),
          "Remote work",
        );
        assert.equal(
          yield* git(remote, ["log", "-1", "--pretty=%s", "feature/collision-2"]),
          "Local work",
        );
      }),
    );
