...
/**
 * Entries safe to share across concurrent app-servers (auth + config + skills).
 * Never share state_*.sqlite / logs_*.sqlite — multiple processes on one
 * CODEX_HOME fail with "failed to initialize sqlite state runtime" (Codex
 * holds an exclusive lock on logs_2.sqlite / state_5.sqlite).
 */
const SESSION_SHARED_LINK_ENTRIES = [
  "auth.json",
  "config.toml",
  "models_cache.json",
  "installation_id",
...
/**
 * Per-thread CODEX_HOME: private SQLite state/logs, shared auth/config via
 * symlinks from the account home. Enables many concurrent Codex app-servers.
 */
export const materializeCodexSessionRuntimeHome = Effect.fn("materializeCodexSessionRuntimeHome")(
  function* (input: {
    readonly sharedHomePath: string;
    readonly sessionHomePath: string;
  }): Effect.fn.Return<string, CodexShadowHomeError, FileSystem.FileSystem | Path.Path> {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const sharedHomePath = path.resolve(input.sharedHomePath);
    const sessionHomePath = path.resolve(input.sessionHomePath);

    if (sharedHomePath === sessionHomePath) {
      return yield* new CodexShadowHomePathConflictError({
        sharedHomePath,
        effectiveHomePath: sessionHomePath,
      });
    }

    yield* fileSystem.makeDirectory(sessionHomePath, { recursive: true }).pipe(
      Effect.catchTags({
        PlatformError: (cause) =>
          new CodexShadowHomeFileSystemError({
            sharedHomePath,
            effectiveHomePath: sessionHomePath,
            operation: "makeDirectory",
            path: sessionHomePath,
            cause,
          }),
      }),
    );

    // Drop any accidental private sqlite links from older broken materializations.
    const existing = yield* fileSystem.readDirectory(sessionHomePath).pipe(
      Effect.catchTags({
        PlatformError: (cause) =>
          new CodexShadowHomeFileSystemError({
            sharedHomePath,
    ...
    for (const entryName of existing) {
      if (
        entryName.startsWith("state_") ||
        entryName.startsWith("logs_") ||
        entryName.startsWith("memories_") ||
        entryName.startsWith("queue_") ||
        entryName.startsWith("goals_")
      ) {
        const linkPath = path.join(sessionHomePath, entryName);
        const state = yield* readLinkState({
          fileSystem,
          sharedHomePath,
          effectiveHomePath: sessionHomePath,
        ...
        if (state._tag === "Symlink") {
          yield* fileSystem.remove(linkPath).pipe(
            Effect.catchTags({
              PlatformError: (cause) =>
                new CodexShadowHomeFileSystemError({
                  sharedHomePath,
    ...
    for (const entryName of SESSION_SHARED_LINK_ENTRIES) {
      const target = path.join(sharedHomePath, entryName);
      const exists = yield* fileSystem.exists(target).pipe(
        Effect.catchTags({
...
/** Shared account home used for auth when settings leave homePath empty. */
export function resolveCodexSharedHomePath(path: Path.Path, homePath: string | undefined): string {
  return resolveHomePath(path, homePath);
}

/** Durable per-thread runtime home under the server base dir. */
export function resolveCodexSessionHomePath(
  path: Path.Path,
  baseDir: string,
  threadId: string,
): string {
  return path.join(path.resolve(baseDir), "codex-sessions", threadId);
}
