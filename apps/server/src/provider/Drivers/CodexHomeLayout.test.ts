...
import {
  ...
  materializeCodexSessionRuntimeHome,
  materializeCodexShadowHome,
  resolveCodexHomeLayout,
  resolveCodexSessionHomePath,
  resolveCodexSharedHomePath,
...
it.layer(NodeServices.layer)("CodexHomeLayout", (it) => {
  ...
  describe("materializeCodexSessionRuntimeHome", () => {
    it.effect("links auth/config but keeps sqlite state private per thread", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const sharedHome = yield* makeTempDir("t3code-codex-shared-");
        const baseDir = yield* makeTempDir("t3code-base-");
        const sessionHome = resolveCodexSessionHomePath(path, baseDir, "thread-abc");

        yield* writeTextFile(path.join(sharedHome, "auth.json"), '{"token":"x"}\n');
        yield* writeTextFile(path.join(sharedHome, "config.toml"), 'model = "gpt-5.6-luna"\n');
        yield* writeTextFile(path.join(sharedHome, "state_5.sqlite"), "shared-state\n");
        yield* writeTextFile(path.join(sharedHome, "logs_2.sqlite"), "shared-logs\n");
        yield* fileSystem.makeDirectory(path.join(sharedHome, "skills"), { recursive: true });

        const resolved = yield* materializeCodexSessionRuntimeHome({
          sharedHomePath: sharedHome,
          sessionHomePath: sessionHome,
        });

        expect(resolved).toBe(sessionHome);
        expect(yield* fileSystem.readLink(path.join(sessionHome, "auth.json"))).toBe(
          path.join(sharedHome, "auth.json"),
        );
        expect(yield* fileSystem.readLink(path.join(sessionHome, "config.toml"))).toBe(
          path.join(sharedHome, "config.toml"),
        );
        expect(yield* fileSystem.readLink(path.join(sessionHome, "skills"))).toBe(
          path.join(sharedHome, "skills"),
        );
        expect(yield* fileSystem.exists(path.join(sessionHome, "state_5.sqlite"))).toBe(false);
        expect(yield* fileSystem.exists(path.join(sessionHome, "logs_2.sqlite"))).toBe(false);

        // Second concurrent home for another thread also gets private state.
        const sessionHomeB = resolveCodexSessionHomePath(path, baseDir, "thread-def");
        yield* materializeCodexSessionRuntimeHome({
          sharedHomePath: sharedHome,
          sessionHomePath: sessionHomeB,
        });
        expect(sessionHomeB).not.toBe(sessionHome);
        expect(yield* fileSystem.exists(path.join(sessionHomeB, "state_5.sqlite"))).toBe(false);
      }),
    );

    it.effect("resolveCodexSharedHomePath uses explicit homePath", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const home = yield* makeTempDir("t3code-codex-home-");
        expect(resolveCodexSharedHomePath(path, home)).toBe(path.resolve(home));
      }),
    );
  });
});
