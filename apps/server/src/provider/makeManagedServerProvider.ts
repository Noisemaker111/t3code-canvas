...
import { readMemoryPressure } from "../health/memoryPressure.ts";
...
> {
  ...
  // Only the TIMER defers under pressure. A settings change or an explicit
  // Recheck still probes: the user is waiting on that answer, and a stale
  // provider list is worse than a spawn. The skip is recorded rather than
  // logged — Settings → VPS is where a recurring one has to be visible.
  const refreshOnSchedule = Effect.gen(function* () {
    const pressure = yield* readMemoryPressure;
    if (!pressure.low) return yield* refreshSnapshot();
    recordDegradation({
      id: "provider.probe.deferred",
      title: "Provider probe deferred while memory was low",
      detail:
        `Skipped a scheduled provider snapshot refresh: ${pressure.detail ?? "memory unreadable"}. ` +
        "Each probe shells out to a provider CLI; running them while the box is " +
        "swapping costs memory nobody asked for.",
    });
  });

  yield* Effect.forever(
    Effect.sleep(input.refreshInterval ?? "60 seconds").pipe(
      Effect.flatMap(() => refreshOnSchedule),
