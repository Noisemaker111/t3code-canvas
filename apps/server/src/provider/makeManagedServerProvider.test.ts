...
// makeManagedServerProvider reads /proc/meminfo to skip its scheduled probe on
// a box that is out of memory. These suites drive the timer directly and never
// want a real filesystem, so a no-op one satisfies the requirement.
const scopedWithFs = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
  Effect.scoped(effect).pipe(Effect.provide(FileSystem.layerNoop({})));
