...
import { resolveClaudePermissionMode } from "../Drivers/ClaudePermissionMode.ts";
import {
...
interface ClaudeTurnState {
  ...
  readonly thinkingBlocks: Map<number, ClaudeThinkingBlockState>;
...
interface ClaudeThinkingBlockState {
  readonly itemId: string;
  text: string;
}
  ...
  const handleStreamEvent = Effect.fn("handleStreamEvent")(function* (
      ...
      if (
        ...
        const thinkingBlock =
          event.delta.type === "thinking_delta"
            ? context.turnState.thinkingBlocks.get(event.index)
            : undefined;
        if (thinkingBlock) {
          thinkingBlock.text += deltaText;
        }
        ...
        const deltaItemId = assistantBlockEntry?.block.itemId ?? thinkingBlock?.itemId;
        const stamp = yield* makeEventStamp();
        yield* offerRuntimeEvent({
          type: "content.delta",
          eventId: stamp.eventId,
          ...
          ...(deltaItemId
            ? {
                itemId: asRuntimeItemId(deltaItemId),
              }
            : {}),
          payload: {
            streamKind,
    ...
    if (event.type === "content_block_start") {
      ...
      if (block.type === "thinking" && context.turnState) {
        const thinkingBlock: ClaudeThinkingBlockState = {
          itemId: yield* randomUUIDv4,
          text: typeof block.thinking === "string" ? block.thinking : "",
        };
        context.turnState.thinkingBlocks.set(index, thinkingBlock);
    ...
    if (event.type === "content_block_stop") {
      ...
      if (thinkingBlock && context.turnState) {
        ...
        if (thinkingBlock.text.trim().length > 0) {
          const thinkingStamp = yield* makeEventStamp();
          yield* offerRuntimeEvent({
            type: "item.completed",
            eventId: thinkingStamp.eventId,
    ...
    function* (input) {
      ...
      if (input.cwd === undefined) {
        return yield* new ProviderAdapterValidationError({
          provider: PROVIDER,
          operation: "startSession",
          issue: `Thread '${input.threadId}' has no cwd; refusing to run the agent in the server's own directory.`,
        });
      }
      ...
      const { permissionMode } = yield* resolveClaudePermissionMode(
        input.runtimeMode,
        claudeEnvironment,
      );
      const settings = {
      ...
      const slicedClaudeExecutable = yield* agentSliceExecutable("claude", claudeBinaryPath).pipe(
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        pathToClaudeCodeExecutable: slicedClaudeExecutable,
