...
it.layer(testLayer)("checkOpenCodeProviderStatus", (it) => {
  ...
  it.effect("skips OpenCode cursor-acp bridge models (use native Cursor/Grok harnesses)", () =>
    Effect.gen(function* () {
      runtimeMock.state.inventory = {
        providerList: {
          connected: ["openai", "cursor-acp"],
          all: [
            {
              id: "openai",
              name: "OpenAI",
      ...
      const snapshot = yield* checkOpenCodeProviderStatus(makeOpenCodeSettings(), process.cwd());
      NodeAssert.equal(
        snapshot.models.some((entry) => entry.slug.startsWith("cursor-acp/")),
        false,
      );
      NodeAssert.ok(snapshot.models.some((entry) => entry.slug === "openai/gpt-5.4"));
    }),
  );
