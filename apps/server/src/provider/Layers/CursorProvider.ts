...
import { resolveCursorAgentBinaryPath } from "../acp/CursorAcpSupport.ts";
...
const CURSOR_ACP_MODEL_DISCOVERY_FAILED_MESSAGE = [
  ...
  `Check the Cursor CLI setup (${CURSOR_CLI_INSTALLATION_DOCS_URL}), restart T3 Code, and try again.`,
].join(" ");
  ...
  Effect.gen(function* () {
    ...
    const acpContext = yield* Layer.build(
      AcpSessionRuntime.layer({
        spawn: {
          command: resolveCursorAgentBinaryPath(cursorSettings.binaryPath, environment),
          args: [
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    `\`${binaryPath}\` is not on PATH.`,
    `Install the Cursor CLI, then restart T3 Code — ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}
...
const runCursorCommand = (
  ...
  Effect.gen(function* () {
    ...
    const spawnCommand = yield* resolveSpawnCommand(
      resolveCursorAgentBinaryPath(cursorSettings.binaryPath, environment),
      args,
      environment ? { env: environment } : {},
    );
  ...
  if (Option.isNone(aboutProbe.success)) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: `Cursor Agent CLI did not answer \`agent about\` within ${Math.round(ABOUT_TIMEOUT_MS / 1000)}s.`,
      },
    });
  }
