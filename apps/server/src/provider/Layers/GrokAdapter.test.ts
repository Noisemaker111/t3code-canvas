...
/**
 * Wait until `predicate` holds, yielding between checks.
 *
 * These tests used to spin a fixed number of `Effect.yieldNow`s after a
 * terminal event and then assert. That is a race: the runtime publishes events
 * from another fiber, so under load the last `content.delta` lands after the
 * spin and the assertion sees a truncated stream ("hello from " instead of
 * "hello from mock"). Waiting on the condition removes the flake without
 * weakening anything — when it never holds, the caller's assertion still fails.
 */
const settleUntil = (predicate: () => boolean, maxYields = 500) =>
  Effect.gen(function* () {
    for (let attempt = 0; attempt < maxYields && !predicate(); attempt += 1) {
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  ...
  it.effect("completes a Grok turn from xAI prompt completion when the prompt RPC hangs", () =>
    Effect.gen(function* () {
      ...
      yield* Deferred.await(turnCompleted);
      const joinedContentFor = (target: typeof threadId) =>
        runtimeEvents
          .filter(
            (event): event is Extract<ProviderRuntimeEvent, { type: "content.delta" }> =>
              event.type === "content.delta" && String(event.threadId) === String(target),
          )
          .map((event) => event.payload.delta)
          .join("");
      yield* settleUntil(() => joinedContentFor(threadId) === "hello from mock");
      const readySessions = yield* adapter.listSessions();
      const readySession = readySessions.find((session) => session.threadId === threadId);
      const turnCompletedEvent = runtimeEvents.find(
        (event): event is Extract<ProviderRuntimeEvent, { type: "turn.completed" }> =>
  ...
  it.effect("retains turn transcript when sendTurn is interrupted after prompt success", () =>
    Effect.gen(function* () {
      ...
      // Interrupting before the turn is even registered tests nothing, and six
      // blind yields is not enough of a settle once this file shares the host
      // with the rest of the suite — that lost the turn entirely. Wait for the
      // observable precondition instead.
      yield* Effect.gen(function* () {
        let current = yield* adapter.readThread(threadId);
        for (let attempt = 0; attempt < 2000 && current.turns.length < 1; attempt += 1) {
          yield* Effect.yieldNow;
          current = yield* adapter.readThread(threadId);
        }
      });
      yield* Fiber.interrupt(sendTurnFiber);
      // The interrupted turn's transcript is written from the adapter's own
      // fiber, so poll the thread for it rather than spinning a fixed number
      // of yields and hoping.
      const snapshot = yield* Effect.gen(function* () {
        let current = yield* adapter.readThread(threadId);
        for (
          let attempt = 0;
          attempt < 2000 && (current.turns[0]?.items.length ?? 0) < 1;
          attempt += 1
        ) {
          yield* Effect.yieldNow;
          current = yield* adapter.readThread(threadId);
