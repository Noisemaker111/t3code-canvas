...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsModule.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            const registry = yield* ProviderRegistry.ProviderRegistry;
            let providers = yield* registry.getProviders;
            // Bounded by event-loop turns, not wall clock, and what it waits on
            // is a real ENOENT spawn — so the bound has to cover the slowest
            // host, not the fastest. It exits the turn the probe lands, so a
            // generous ceiling costs nothing and is what keeps this green when
            // the file runs alongside the rest of the suite.
            for (
              let attempts = 0;
              attempts < 2000 &&
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("trusts an explicit logged-out CLI when initialization is unavailable", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
