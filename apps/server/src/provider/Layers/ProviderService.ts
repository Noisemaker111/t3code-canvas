...
const makeProviderService = Effect.fn("makeProviderService")(function* (
    ...
    adapter.startSession(input).pipe(
      Effect.catchTag("ProviderAdapterProcessError", (error) => {
        if (
          ...
          !/no rollout found/i.test(error.detail)
        ) {
          return Effect.fail(error);
        }
        const { resumeCursor: _staleResumeCursor, ...freshInput } = input;
        return Effect.logWarning("Codex resume state is missing; starting a fresh session", {
          threadId: input.threadId,
        }).pipe(Effect.andThen(adapter.startSession(freshInput)));
      }),
    );
  ...
  const recoverSessionForThread = Effect.fn("recoverSessionForThread")(function* (input: {
    ...
    return yield* Effect.gen(function* () {
      ...
      const resumed = yield* startAdapterSession(adapter, {
  ...
  const startSession: ProviderServiceMethod<"startSession"> = Effect.fn("startSession")(
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        const session = yield* startAdapterSession(adapter, {
          ...input,
