...
const TEST_CWD = "/tmp/claude-adapter-test";
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("emits a reasoning item lifecycle for streamed thinking blocks", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      const reasoningStarted = runtimeEvents.find((event) => event.type === "item.started");
      ...
      if (reasoningStarted?.type === "item.started") {
        ...
        assert.equal(String(reasoningStarted.turnId), String(turn.turnId));
      }

      const reasoningCompleted = runtimeEvents.find((event) => event.type === "item.completed");
      ...
      if (reasoningCompleted?.type === "item.completed") {
        ...
        assert.equal(reasoningCompleted.payload.detail, "Weighing the options.");
      ...
      for (const delta of reasoningDeltas) {
        assert.equal(String(delta.itemId), String(reasoningStarted?.itemId));
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
