...
/**
 * OpenCode can expose a `cursor-acp/*` bridge (open-cursor plugin → cursor-agent).
 * T3 already has first-class Cursor + Grok ACP drivers; selecting Grok through
 * OpenCode→cursor-acp double-wraps the agent (tool-loop thrash, Windows .cmd
 * spawn EINVAL without a PE shim). Skip that catalog in T3 — use harness
 * Cursor/Grok instead.
 */
const OPENCODE_SKIPPED_BRIDGE_PROVIDER_IDS = new Set(["cursor-acp"]);
...
function flattenOpenCodeModels(input: OpenCodeInventory): ReadonlyArray<ServerProviderModel> {
  ...
  for (const provider of input.providerList.all) {
    ...
    if (OPENCODE_SKIPPED_BRIDGE_PROVIDER_IDS.has(provider.id)) {
      continue;
    }
