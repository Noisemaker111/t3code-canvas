  ...
  const startSession: CodexAdapterShape["startSession"] = (input) =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        // Concurrent app-servers cannot share one CODEX_HOME: Codex takes an
        // exclusive lock on state_*.sqlite / logs_*.sqlite and the second
        // process dies with "failed to initialize sqlite state runtime".
        // Per-thread homes keep auth linked from the account home.
        const sharedCodexHome = resolveCodexSharedHomePath(pathService, codexConfig.homePath);
        const sessionCodexHome = resolveCodexSessionHomePath(
          pathService,
        ...
        yield* materializeCodexSessionRuntimeHome({
          ...
          sessionHomePath: sessionCodexHome,
        }).pipe(
