...
describe("ProviderSessionReaper", () => {
  ...
  async function createHarness(input: {
    ...
    const layer = makeProviderSessionReaperLive({
      ...
      // Explicit so a test that predates the stuck-turn watchdog is not
      // silently reinterpreted by it: timestamps here are fixed in the past.
      stuckTurnThresholdMs: input.stuckTurnThresholdMs ?? 10 * 365 * 24 * 60 * 60 * 1000,
    }).pipe(
      Layer.provideMerge(
  ...
  it("settles a turn that wedged, because nothing else ever will", async () => {
    const threadId = ThreadId.make("thread-reaper-stuck-turn");
    const turnId = TurnId.make("turn-reaper-stuck");
    const harness = await createHarness({
      stuckTurnThresholdMs: 1_000,
      readModel: makeReadModel([
        {
          id: threadId,
          session: {
    ...
    await runtime!.runPromise(
      repository.upsert({
        ...
        resumeCursor: { opaque: "resume-stuck-turn" },
        runtimePayload: null,
      }),
    );
    ...
    // The turn is settled first, so the thread stops reading as "Working for…".
    const settle = harness.dispatched.find((entry) => entry.type === "thread.session.set");
    expect(settle).toBeDefined();
    const session = settle?.session as {
      status?: string;
    ...
    expect(session?.lastError).toContain("no sign of life");
