...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("starts a fresh Codex session when persisted rollout state is missing", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-missing-rollout");

      const initial = yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
        cwd: "/tmp/project-missing-rollout",
        runtimeMode: "full-access",
      });
      yield* provider.stopSession({ threadId });
      routing.codex.startSession.mockClear();
      routing.codex.startSession.mockImplementationOnce((input: ProviderSessionStartInput) =>
        Effect.fail(
          new ProviderAdapterProcessError({
            provider: "codex",
            threadId: input.threadId,
            detail: "no rollout found for persisted thread",
          }),
        ),
      );
      routing.codex.startSession.mockImplementationOnce((input: ProviderSessionStartInput) =>
        Effect.succeed({
          ...
          runtimeMode: input.runtimeMode,
          threadId: input.threadId,
          resumeCursor: { opaque: "fresh-rollout" },
          cwd: input.cwd ?? process.cwd(),
          createdAt: "2026-01-01T00:00:00.000Z",
      ...
      assert.equal("resumeCursor" in (routing.codex.startSession.mock.calls[1]?.[0] ?? {}), false);
      assert.equal(recovered.threadId, threadId);
      assert.deepEqual(recovered.resumeCursor, { opaque: "fresh-rollout" });
    }),
  );
