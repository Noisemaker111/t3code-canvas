...
// What this waits on is the ACP child process flushing a line to disk, but the
// bound counts event-loop turns — so a loaded host burns through it while the
// write is still in flight. It returns the turn the predicate matches, so the
// ceiling only has to cover the slowest host, not the fastest.
