...
import { parseClaudeAuthLoggedIn } from "../claudeAuth.ts";
...
export { parseClaudeAuthLoggedIn } from "../claudeAuth.ts";
...
> {
  ...
  if (Option.isNone(versionProbe.success)) {
    return buildServerProvider({
      ...
      checkedAt,
      models: allModels,
      // A slow `--version` is a busy box, not a broken CLI. `error` took the
      // instance out of every model picker until some later probe happened to
      // come back in time; `warning` says so and leaves it pickable.
      probe: {
        installed: true,
        version: null,
        status: "warning",
        auth: { status: "unknown" },
        message: `Claude Agent CLI did not answer \`claude --version\` within ${Math.round(DEFAULT_TIMEOUT_MS / 1000)}s.`,
      },
    });
  }
  ...
  // CLI truth wins over SDK init: credentials files can exist while
  // `claude auth status` reports loggedIn:false (expired OAuth). Without this
  // the Models UI stays green and Log in never appears, while every Active
  // Claude turn dies with "OAuth session expired".
  const authStatusProbe = yield* runClaudeCommand(
    claudeSettings,
    ["auth", "status"],
    resolvedEnvironment,
  ).pipe(Effect.timeoutOption(DEFAULT_TIMEOUT_MS), Effect.result);
  const cliLoggedIn = Result.isSuccess(authStatusProbe)
    ? Option.isSome(authStatusProbe.success)
      ? parseClaudeAuthLoggedIn(
          `${authStatusProbe.success.value.stdout}\n${authStatusProbe.success.value.stderr}`,
        )
      : undefined
    : undefined;
  ...
  if (cliLoggedIn === false) {
    return buildServerProvider({
      ...
      probe: {
        ...
        message:
          "Claude login expired — click Log in (or run `claude auth login` in the host terminal).",
      },
    });
  }
  ...
  if (!capabilities) {
    return buildServerProvider({
      ...
      probe: {
        ...
        version: parsedVersion,
        status: "warning",
        auth: { status: cliLoggedIn === true ? "authenticated" : "unknown" },
        message:
          cliLoggedIn === true
            ? "Claude is logged in, but subscription details could not be read from the SDK."
