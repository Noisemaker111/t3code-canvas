...
const makeOpenCodeRuntime = Effect.gen(function* () {
  ...
  const runOpenCodeCommand: OpenCodeRuntimeShape["runOpenCodeCommand"] = (input) =>
    Effect.gen(function* () {
      // Probe CLIs (`--version`, `models --verbose`, `agent list`) are agent
      // work too: they load the whole provider inventory. Unsliced they ran in
      // the server's own cgroup, and on 3 Aug 2026 four of them stalled in
      // mem_cgroup_handle_over_high and throttled the board with them.
      const spawnCommand = inAgentSlice(
        yield* resolveCommand(input.binaryPath, input.args, input.environment),
      );
      ...
      // Probe inventory commands have hung for tens of minutes on a busy box
      // (`opencode --version`, `models --verbose`, `agent list`) and held a
      // core outside any caller timeout. Cap every CLI run here.
      const OPENCODE_CLI_TIMEOUT_MS = 30_000;
      const result = yield* Effect.all(
        [collectStreamAsString(child.stdout), collectStreamAsString(child.stderr), child.exitCode],
        { concurrency: "unbounded" },
      ).pipe(
        Effect.timeoutOption(OPENCODE_CLI_TIMEOUT_MS),
        Effect.flatMap(
          Option.match({
            onNone: () =>
              Effect.fail(
                new OpenCodeRuntimeError({
                  ...
                  detail: `'${input.binaryPath} ${input.args.join(" ")}' did not finish within ${OPENCODE_CLI_TIMEOUT_MS / 1000}s`,
                }),
              ),
  ...
  const startOpenCodeServerProcess: OpenCodeRuntimeShape["startOpenCodeServerProcess"] = (input) =>
    Effect.gen(function* () {
      ...
      const spawnCommand = inAgentSlice(
        yield* resolveCommand(input.binaryPath, args, input.environment),
      );
  ...
  const loadInventoryFromCli: OpenCodeRuntimeShape["loadInventoryFromCli"] = (input) =>
    Effect.gen(function* () {
      ...
      // A failed probe used to return an empty inventory, which reads as "this
      // provider has no models" — a silent capability downgrade. Fail instead
      // and let checkOpenCodeProviderStatus surface the real reason.
      const describeFailure = (
        label: string,
        result: Exit.Exit<OpenCodeCommandResult, OpenCodeRuntimeError>,
      ): string | null => {
        if (Exit.isFailure(result)) {
          return `${label}: ${openCodeRuntimeErrorDetail(Cause.squash(result.cause))}`;
        }
        if (result.value.code === 0) return null;
        const firstStderrLine = result.value.stderr.trim().split("\n")[0] ?? "";
        return `${label}: exited ${result.value.code}${firstStderrLine ? ` — ${firstStderrLine}` : ""}`;
      };
      const probeFailures = [
        describeFailure("opencode models --verbose", modelsResult),
        describeFailure("opencode agent list", agentsResult),
      ].filter((failure): failure is string => failure !== null);
      if (probeFailures.length > 0) {
        return yield* new OpenCodeRuntimeError({
          operation: "loadInventoryFromCli",
          detail: probeFailures.join("; "),
        });
      }
