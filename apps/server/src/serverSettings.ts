...
// A `ModelSelection` needs both `instanceId` and `model`, so stripping either
// half for matching the default writes a partial object that fails to encode.
...
const make = Effect.gen(function* () {
  ...
  /**
   * `trusted` is false when the file is there but could not be read as
   * settings. The server still boots on defaults, but `updateSettings` must
   * not persist them: one unreadable read followed by any client patch used to
   * overwrite the whole file with defaults, permanently losing model roles,
   * provider config and skill commands.
   */
  const loadSettingsFromDisk = Effect.gen(function* () {
    if (!(yield* readConfigExists)) {
      return { settings: DEFAULT_SERVER_SETTINGS, trusted: true };
    }

    const raw = yield* readRawConfig;
    if (raw.trim().length === 0) {
      yield* Effect.logWarning("settings.json is empty, using defaults without persisting", {
        path: settingsPath,
      });
      return { settings: DEFAULT_SERVER_SETTINGS, trusted: false };
    }
    ...
    return { settings: decoded.value, trusted: true };
  });

  const settingsCache = yield* Cache.make<
    typeof cacheKey,
    { readonly settings: ServerSettings; readonly trusted: boolean },
    ServerSettingsError
  >({
    capacity: 1,
  ...
  const getLoadedSettings = Cache.get(settingsCache, cacheKey);
  const getSettingsFromCache = getLoadedSettings.pipe(Effect.map((loaded) => loaded.settings));
  ...
  return {
    ...
    updateSettings: (patch) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          const loaded = yield* getLoadedSettings;
          if (!loaded.trusted) {
            return yield* Effect.fail(
              new ServerSettingsError({
                ...
                operation: "read-file",
                cause: new Error(
                  `Refusing to write ${settingsPath}: its current contents could not be read, so patching would replace real settings with defaults. Fix or remove the file, then retry.`,
                ),
              }),
            );
          }
          ...
          yield* Cache.set(settingsCache, cacheKey, { settings: next, trusted: true });
