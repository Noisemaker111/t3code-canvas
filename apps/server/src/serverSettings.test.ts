...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("replaces the whole skillCommands map rather than merging entries", () =>
    Effect.gen(function* () {
      const serverSettings = yield* ServerSettingsModule.ServerSettingsService;
      const handoffId = SkillCommandId.make("handoff");
      const reviewId = SkillCommandId.make("review");

      yield* serverSettings.updateSettings({
        skillCommands: {
          [handoffId]: { prompt: "Summarize where we left off." },
          [reviewId]: { prompt: "Review the current diff for bugs." },
        },
      });

      const next = yield* serverSettings.updateSettings({
        skillCommands: {
          [handoffId]: { prompt: "Updated handoff prompt." },
        },
      });

      assert.deepEqual(next.skillCommands, {
        [handoffId]: { prompt: "Updated handoff prompt." },
      });
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );
  ...
  it.effect("refuses to overwrite a settings.json it could not read", () =>
    Effect.gen(function* () {
      const serverConfig = yield* ServerConfig.ServerConfig;
      const fileSystem = yield* FileSystem.FileSystem;
      // A half-written / truncated file must not read as "no settings" —
      // patching on top of that used to persist defaults over the whole file.
      const corrupt = '{ "defaultModelSelection": { "instanceId": "claudeAgent"';
      yield* fileSystem.writeFileString(serverConfig.settingsPath, corrupt);

      const serverSettings = yield* ServerSettingsModule.ServerSettingsService;
      const failure = yield* serverSettings
        .updateSettings({ addProjectBaseDirectory: "~/Development" })
        .pipe(Effect.flip);
      assert.equal(failure._tag, "ServerSettingsError");

      assert.equal(yield* fileSystem.readFileString(serverConfig.settingsPath), corrupt);
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );

  it.effect("persists a default model selection that shares one field with the default", () =>
    Effect.gen(function* () {
      const serverSettings = yield* ServerSettingsModule.ServerSettingsService;
      const serverConfig = yield* ServerConfig.ServerConfig;
      const fileSystem = yield* FileSystem.FileSystem;
      ...
      yield* serverSettings.updateSettings({
        defaultModelSelection: {
          instanceId: DEFAULT_SERVER_SETTINGS.defaultModelSelection.instanceId,
          model: "gpt-5.4-codex",
        },
      });

      const raw = yield* fileSystem.readFileString(serverConfig.settingsPath);
      const reloaded = yield* decodeServerSettings(
        // @effect-diagnostics-next-line preferSchemaOverJson:off
        JSON.parse(raw) as unknown,
      );

      assert.deepEqual(reloaded.defaultModelSelection, {
        instanceId: DEFAULT_SERVER_SETTINGS.defaultModelSelection.instanceId,
        model: "gpt-5.4-codex",
      });
    }).pipe(Effect.provide(makeServerSettingsLayer())),
