...
import { clearSessionErrorWhenReady } from "./threadHygiene.ts";
  ...
  switch (event.type) {
      ...
      return decodeForEvent(ProjectCreatedPayload, event.payload, event.type, "payload").pipe(
        Effect.map((payload) => {
          ...
          const nextProject = {
            ...
            ...(payload.defaultBaseBranch !== undefined
              ? { defaultBaseBranch: payload.defaultBaseBranch }
              : {}),
    ...
    case "thread.session-set":
      return Effect.gen(function* () {
        ...
        const session = clearSessionErrorWhenReady(decodedSession);
