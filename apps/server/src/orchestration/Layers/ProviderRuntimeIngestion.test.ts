...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("measures the tool span from item.started to item.completed", async () => {
    const harness = await createHarness();

    harness.emit({
      type: "item.started",
      eventId: asEventId("evt-span-started"),
      provider: ProviderDriverKind.make("claudeAgent"),
      createdAt: "2026-01-01T00:00:01.000Z",
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-span"),
      itemId: asItemId("item-span"),
      payload: {
    ...
    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-span-completed"),
      provider: ProviderDriverKind.make("claudeAgent"),
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) => activity.id === "evt-span-completed",
      ),
    );
    const activity = thread.activities.find(
      (entry: ProviderRuntimeTestActivity) => entry.id === "evt-span-completed",
    );
    const payload = activity?.payload as Record<string, unknown> | undefined;

    expect(payload?.itemId).toBe("item-span");
    expect(payload?.startedAt).toBe("2026-01-01T00:00:01.000Z");
    expect(payload?.durationMs).toBe(8_000);
    expect(typeof payload?.tokens).toBe("number");
  });

  it("projects completed reasoning items as thought activities with tokens and time", async () => {
    const harness = await createHarness();

    harness.emit({
      type: "item.started",
      eventId: asEventId("evt-think-started"),
      provider: ProviderDriverKind.make("claudeAgent"),
      createdAt: "2026-01-01T00:00:02.000Z",
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-think"),
      itemId: asItemId("item-think"),
      payload: {
        itemType: "reasoning",
        status: "inProgress",
        title: "Thought",
    ...
    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-think-completed"),
      provider: ProviderDriverKind.make("claudeAgent"),
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) => activity.id === "evt-think-completed",
      ),
    );
    const activity = thread.activities.find(
      (entry: ProviderRuntimeTestActivity) => entry.id === "evt-think-completed",
    );
    const payload = activity?.payload as Record<string, unknown> | undefined;

    expect(activity?.kind).toBe("thought");
    expect(activity?.summary).toBe("Weighing the options before editing.");
    expect(payload?.durationMs).toBe(6_000);
    expect(payload?.tokens as number).toBeGreaterThan(0);
  });

  it("records message.metrics with the completed assistant message's token count", async () => {
    const harness = await createHarness();
    ...
    harness.emit({
      ...
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-assistant"),
      itemId: asItemId("item-assistant"),
      payload: {
        itemType: "assistant_message",
        status: "completed",
        title: "Assistant message",
        detail: "Here is the final answer with enough words to count.",
      },
    });
    ...
    const thread = await waitForThread(harness.readModel, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) =>
          activity.id === "evt-assistant-completed:message-metrics",
      ),
    );
    const activity = thread.activities.find(
      (entry: ProviderRuntimeTestActivity) =>
        entry.id === "evt-assistant-completed:message-metrics",
    );
    const payload = activity?.payload as Record<string, unknown> | undefined;

    expect(activity?.kind).toBe("message.metrics");
    expect(payload?.messageId).toBe("assistant:item-assistant");
    expect(payload?.tokens as number).toBeGreaterThan(0);
  });
