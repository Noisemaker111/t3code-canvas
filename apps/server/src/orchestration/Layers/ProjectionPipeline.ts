...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
    ...
    const settleStreamingThreadMessages = Effect.fn("settleStreamingThreadMessages")(function* (
      threadId: ThreadId,
      updatedAt: string,
    ) {
      ...
      switch (event.type) {
        case "project.created":
          yield* projectionProjectRepository.upsert({
            ...
            defaultBaseBranch: event.payload.defaultBaseBranch ?? null,
        ...
        case "project.meta-updated": {
          ...
          yield* projectionProjectRepository.upsert({
            ...
            ...(event.payload.defaultModelSelection !== undefined
              ? { defaultModelSelection: event.payload.defaultModelSelection }
              : {}),
            ...(event.payload.defaultBaseBranch !== undefined
              ? { defaultBaseBranch: event.payload.defaultBaseBranch }
              : {}),
    ...
    const applyThreadMessagesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.session-set": {
          if (settledTurnStateForSessionStatus(event.payload.session.status) === null) {
            return;
          }
          yield* settleStreamingThreadMessages(
        ...
        case "thread.archived":
        case "thread.deleted": {
          yield* settleStreamingThreadMessages(event.payload.threadId, event.occurredAt);
          return;
        }
      ...
      switch (event.type) {
        ...
        case "thread.turn-interrupt-requested": {
          if (event.payload.turnId === undefined) {
            // The client only knows a turn id while the session reports one, so
            // a stop on a thread whose session is stale arrives without it.
            // Settle whatever is still running instead of dropping the stop.
            const runningTurns = yield* projectionTurnRepository.listByThreadId({
              threadId: event.payload.threadId,
            });
            yield* Effect.forEach(
              runningTurns.filter((turn) => turn.turnId !== null && turn.state === "running"),
              (turn) =>
                turn.turnId === null
                  ? Effect.void
                  : projectionTurnRepository.upsertByTurnId({
