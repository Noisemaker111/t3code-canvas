...
/** Milliseconds between two ISO timestamps, or undefined when unparseable/negative. */
function elapsedMsBetween(startIso: string, endIso: string): number | undefined {
...
/**
 * Cumulative output tokens from a task/turn usage payload. Providers report
 * either SDK-raw (`output_tokens`) or normalized (`outputTokens`) shapes.
 */
function readCumulativeOutputTokens(usage: unknown): number | undefined {
  if (!usage || typeof usage !== "object") {
    return undefined;
  }
  const record = usage as Record<string, unknown>;
  for (const key of ["outputTokens", "output_tokens"]) {
...
/**
 * Timing/usage facts the stateful ingestion loop attributes to a single
 * event's activities: the matching lifecycle start (`item.started` /
 * `task.started`) and, for tasks, the output tokens this step consumed.
 * Persisted into the activity payload so every client renders the same
 * authoritative numbers instead of re-deriving them from row gaps.
 */
...
/**
 * Count the tokens a completed tool call consumed. Harnesses do not attribute
 * tokens per tool call, so we tokenize the call's own text — its title, detail
 * (e.g. a command's output), and structured data (its input) — with a real BPE
 * tokenizer. Computed once here at ingestion, off the agent and off the UI.
 */
function computeToolActivityTokens(input: {
  ...
  if (input.data !== undefined) {
    try {
      parts.push(typeof input.data === "string" ? input.data : JSON.stringify(input.data));
    } catch {
      // Ignore data that cannot be serialized (e.g. cyclic structures).
    }
  }
...
export function runtimeEventToActivities(
  ...
  const timingFields = {
    ...(enrichment?.startedAt ? { startedAt: enrichment.startedAt } : {}),
    ...(enrichment?.durationMs !== undefined ? { durationMs: enrichment.durationMs } : {}),
  };
  const maybeSequence = (() => {
  ...
  switch (event.type) {
      ...
      return [
        {
          ...
          payload: {
            ...
            ...(enrichment?.taskTokens !== undefined && enrichment.taskTokens > 0
              ? { tokens: enrichment.taskTokens }
              : {}),
          },
          turnId: toTurnId(event.turnId) ?? null,
          ...maybeSequence,
    ...
    case "item.updated": {
      ...
      // Some providers end a tool call on item.updated with a terminal status
      // and never emit item.completed — count tokens and close the span here
      // too, so those rows aren't left without metrics.
      const terminal =
        event.payload.status === "completed" ||
        event.payload.status === "failed" ||
        event.payload.status === "declined";
      const updateTokens = terminal
        ? computeToolActivityTokens({
      ...
      return [
        {
          ...
          payload: {
            ...
            ...(updateTokens > 0 ? { tokens: updateTokens } : {}),
            ...(terminal ? timingFields : {}),
          },
          turnId: toTurnId(event.turnId) ?? null,
          ...maybeSequence,
        },
    ...
    case "item.completed": {
      if (event.payload.itemType === "reasoning") {
        const thoughtText = event.payload.detail?.trim();
        if (!thoughtText) {
          return [];
        }
        const thoughtTokens = countTokens(thoughtText);
        return [
          {
            id: event.eventId,
            createdAt: event.createdAt,
            tone: "info",
            kind: "thought",
            summary: truncateDetail(firstNonEmptyLine(thoughtText) ?? "Thought", 120),
            payload: {
              itemType: "reasoning",
              detail: truncateDetail(thoughtText, 2000),
              ...(event.itemId ? { itemId: event.itemId } : {}),
              ...(thoughtTokens > 0 ? { tokens: thoughtTokens } : {}),
              ...timingFields,
            },
            turnId: toTurnId(event.turnId) ?? null,
            ...maybeSequence,
      ...
      const toolTokens = computeToolActivityTokens({
        title: event.payload.title,
        detail: event.payload.detail,
        data: event.payload.data,
      });
      return [
        {
          ...
          payload: {
            ...
            ...(event.payload.detail ? { detail: truncateDetail(event.payload.detail) } : {}),
            ...(event.payload.data !== undefined ? { data: event.payload.data } : {}),
            ...(toolTokens > 0 ? { tokens: toolTokens } : {}),
            ...timingFields,
          },
          turnId: toTurnId(event.turnId) ?? null,
          ...maybeSequence,
...
const make = Effect.gen(function* () {
  ...
  // Lifecycle start times: item.started / task.started timestamps keyed by
  // thread+id, so terminal events can be stamped with an authoritative span
  // instead of every client re-deriving durations from row gaps.
  const itemStartedAtByKey = yield* Cache.make<string, string>({
    capacity: TASK_DESCRIPTION_BY_TASK_CACHE_CAPACITY,
    timeToLive: TASK_DESCRIPTION_BY_TASK_TTL,
    lookup: () => Effect.succeed(""),
  });

  const taskStartedAtByKey = yield* Cache.make<string, string>({
    capacity: TASK_DESCRIPTION_BY_TASK_CACHE_CAPACITY,
    timeToLive: TASK_DESCRIPTION_BY_TASK_TTL,
    lookup: () => Effect.succeed(""),
  });

  // Last-seen cumulative output tokens per task, to attribute each
  // task.progress row the delta it actually consumed.
  const taskOutputTokensByKey = yield* Cache.make<string, number>({
    capacity: TASK_DESCRIPTION_BY_TASK_CACHE_CAPACITY,
    timeToLive: TASK_DESCRIPTION_BY_TASK_TTL,
    lookup: () => Effect.succeed(-1),
  });

  const lookupStartedAt = (cache: Cache.Cache<string, string>, key: string) =>
    Cache.getOption(cache, key).pipe(
      Effect.map((value) =>
        Option.filter(value, (iso) => iso.length > 0).pipe(Option.getOrUndefined),
      ),
    );

  /**
   * Timing/token facts for this event's activities, from lifecycle state the
   * serial ingestion worker tracks across events. Also maintains that state
   * (records starts, clears finished items/tasks).
   */
  const resolveActivityEnrichment = Effect.fn("resolveActivityEnrichment")(function* (
    threadId: ThreadId,
    event: ProviderRuntimeEvent,
  ) {
    switch (event.type) {
      case "item.started": {
        if (event.itemId) {
          yield* Cache.set(
            itemStartedAtByKey,
            providerTaskKey(threadId, event.itemId),
            event.createdAt,
          );
        }
        return undefined;
      ...
      case "item.completed": {
        ...
        const key = providerTaskKey(threadId, event.itemId);
        const startedAt = yield* lookupStartedAt(itemStartedAtByKey, key);
        if (event.type === "item.completed") {
          yield* Cache.invalidate(itemStartedAtByKey, key);
        }
        if (!startedAt) {
          return undefined;
        }
        const durationMs = elapsedMsBetween(startedAt, event.createdAt);
        return { startedAt, ...(durationMs !== undefined ? { durationMs } : {}) };
      }
      case "task.started": {
        const key = providerTaskKey(threadId, event.payload.taskId);
        yield* Cache.set(taskStartedAtByKey, key, event.createdAt);
        yield* Cache.invalidate(taskOutputTokensByKey, key);
        return undefined;
      }
      case "task.progress": {
        const key = providerTaskKey(threadId, event.payload.taskId);
        const cumulative = readCumulativeOutputTokens(event.payload.usage);
        if (cumulative === undefined) {
          return undefined;
        }
        const previous = Option.getOrUndefined(yield* Cache.getOption(taskOutputTokensByKey, key));
        yield* Cache.set(taskOutputTokensByKey, key, cumulative);
        const delta = previous !== undefined && previous >= 0 ? cumulative - previous : cumulative;
        return delta > 0 ? { taskTokens: delta } : undefined;
      }
      case "task.completed": {
        const key = providerTaskKey(threadId, event.payload.taskId);
        const startedAt = yield* lookupStartedAt(taskStartedAtByKey, key);
        const lastSeen = Option.getOrUndefined(yield* Cache.getOption(taskOutputTokensByKey, key));
        yield* Cache.invalidate(taskStartedAtByKey, key);
        yield* Cache.invalidate(taskOutputTokensByKey, key);
        const cumulative =
          readCumulativeOutputTokens(event.payload.usage) ??
          (lastSeen !== undefined && lastSeen >= 0 ? lastSeen : undefined);
        const durationMs = startedAt ? elapsedMsBetween(startedAt, event.createdAt) : undefined;
        if (cumulative === undefined && durationMs === undefined) {
          return undefined;
        }
        return {
          ...(startedAt ? { startedAt } : {}),
          ...(durationMs !== undefined ? { durationMs } : {}),
          ...(cumulative !== undefined && cumulative > 0 ? { taskTokens: cumulative } : {}),
        };
      }
      default:
        return undefined;
    ...
    Effect.gen(function* () {
      ...
      if (assistantCompletion) {
        ...
        if (!shouldSkipRedundantCompletion) {
          ...
          // Count the finished message's own text once, server-side, and keep
          // it as an activity keyed by message id — per-turn usage snapshots
          // cannot attribute tokens to an individual message.
          const completionText =
            (assistantCompletion.fallbackText?.trim().length ?? 0) > 0
              ? (assistantCompletion.fallbackText as string)
              : (existingAssistantMessage?.text ?? "");
          const messageTokens = countTokens(completionText);
          if (messageTokens > 0) {
            yield* orchestrationEngine.dispatch({
              type: "thread.activity.append",
              commandId: yield* providerCommandId(event, "message-metrics-append"),
              threadId: thread.id,
              activity: {
                id: EventId.make(`${event.eventId}:message-metrics`),
                createdAt: event.createdAt,
                tone: "info",
                kind: "message.metrics",
                summary: "Assistant message metrics",
                payload: { messageId: assistantMessageId, tokens: messageTokens },
                turnId: turnId ?? null,
              },
              createdAt: event.createdAt,
            });
      ...
      const enrichment = yield* resolveActivityEnrichment(thread.id, event);
      const activities = runtimeEventToActivities(event, taskTitle, enrichment);
