...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("clears pending starts on failure, terminal session, or archive", () =>
    Effect.gen(function* () {
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const requestedAt = "2026-02-26T13:00:00.000Z";
      const terminalAt = "2026-02-26T13:00:05.000Z";
      const appendAndProject = (event: Parameters<typeof eventStore.append>[0]) =>
        eventStore
          .append(event)
          .pipe(Effect.flatMap((savedEvent) => projectionPipeline.projectEvent(savedEvent)));
      const requestPending = (threadId: ThreadId, suffix: string) =>
        appendAndProject({
          type: "thread.turn-start-requested",
          eventId: EventId.make(`evt-pending-${suffix}-request`),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: requestedAt,
          commandId: CommandId.make(`cmd-pending-${suffix}-request`),
          causationEventId: null,
          correlationId: CorrelationId.make(`cmd-pending-${suffix}-request`),
          metadata: {},
          payload: {
            threadId,
      ...
      const failedThreadId = ThreadId.make("thread-pending-failed");
      yield* requestPending(failedThreadId, "failed");
      yield* appendAndProject({
        type: "thread.activity-appended",
        eventId: EventId.make("evt-pending-failed-activity"),
        aggregateKind: "thread",
        aggregateId: failedThreadId,
        occurredAt: terminalAt,
        commandId: CommandId.make("cmd-pending-failed-activity"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-pending-failed-activity"),
        metadata: {},
        payload: {
          threadId: failedThreadId,
          activity: {
            id: EventId.make("activity-pending-failed"),
            tone: "error",
            kind: "provider.turn.start.failed",
            summary: "Provider turn start failed",
            payload: { detail: "provider unavailable" },
            turnId: null,
      ...
      const terminalThreadId = ThreadId.make("thread-pending-terminal");
      yield* requestPending(terminalThreadId, "terminal");
      yield* appendAndProject({
        type: "thread.session-set",
        eventId: EventId.make("evt-pending-terminal-session"),
        aggregateKind: "thread",
        aggregateId: terminalThreadId,
        occurredAt: terminalAt,
        commandId: CommandId.make("cmd-pending-terminal-session"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-pending-terminal-session"),
        metadata: {},
        payload: {
      ...
      const archivedThreadId = ThreadId.make("thread-pending-archived");
      yield* requestPending(archivedThreadId, "archived");
      yield* appendAndProject({
        type: "thread.archived",
        eventId: EventId.make("evt-pending-archived"),
        aggregateKind: "thread",
        aggregateId: archivedThreadId,
        occurredAt: terminalAt,
        commandId: CommandId.make("cmd-pending-archived"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-pending-archived"),
        metadata: {},
        payload: {
      ...
      const startingThreadId = ThreadId.make("thread-pending-starting");
      yield* requestPending(startingThreadId, "starting");
      yield* appendAndProject({
        type: "thread.session-set",
        eventId: EventId.make("evt-pending-starting-session"),
        aggregateKind: "thread",
        aggregateId: startingThreadId,
        occurredAt: terminalAt,
        commandId: CommandId.make("cmd-pending-starting-session"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-pending-starting-session"),
        metadata: {},
        payload: {
      ...
      const pendingRows = yield* sql<{ readonly threadId: string }>`
        ...
        ORDER BY thread_id
      `;
      assert.deepStrictEqual(pendingRows, [{ threadId: "thread-pending-starting" }]);
    }),
  );

  it.effect("settles streaming messages when their session ends", () =>
    Effect.gen(function* () {
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const streamingAt = "2026-02-26T13:10:00.000Z";
      const sessionAt = "2026-02-26T13:10:05.000Z";
      const appendAndProject = (event: Parameters<typeof eventStore.append>[0]) =>
        eventStore
          .append(event)
          .pipe(Effect.flatMap((savedEvent) => projectionPipeline.projectEvent(savedEvent)));
      const appendStreamingMessage = (threadId: ThreadId, suffix: string) =>
        appendAndProject({
          type: "thread.message-sent",
          eventId: EventId.make(`evt-streaming-${suffix}-message`),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: streamingAt,
          commandId: CommandId.make(`cmd-streaming-${suffix}-message`),
          causationEventId: null,
          correlationId: CorrelationId.make(`cmd-streaming-${suffix}-message`),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.make(`message-streaming-${suffix}`),
            role: "assistant",
      ...
      const setSession = (threadId: ThreadId, suffix: string, status: "ready" | "starting") =>
        appendAndProject({
          type: "thread.session-set",
          eventId: EventId.make(`evt-streaming-${suffix}-session`),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: sessionAt,
          commandId: CommandId.make(`cmd-streaming-${suffix}-session`),
          causationEventId: null,
          correlationId: CorrelationId.make(`cmd-streaming-${suffix}-session`),
          metadata: {},
          payload: {
            threadId,
            session: {
      ...
      const terminalThreadId = ThreadId.make("thread-streaming-terminal");
      yield* appendStreamingMessage(terminalThreadId, "terminal");
      yield* setSession(terminalThreadId, "terminal", "ready");

      const startingThreadId = ThreadId.make("thread-streaming-starting");
      yield* appendStreamingMessage(startingThreadId, "starting");
      yield* setSession(startingThreadId, "starting", "starting");
      ...
      const rows = yield* sql<{ readonly messageId: string; readonly isStreaming: number }>`
        ...
        FROM projection_thread_messages
        WHERE message_id LIKE 'message-streaming-%'
        ORDER BY message_id
      `;
      assert.deepStrictEqual(rows, [
        { messageId: "message-streaming-starting", isStreaming: 1 },
        { messageId: "message-streaming-terminal", isStreaming: 0 },
      ]);
    }),
  );
});
...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("settles running turns when an interrupt arrives without a turn id", () =>
    Effect.gen(function* () {
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const now = "2026-01-01T00:00:00.000Z";
      const threadId = ThreadId.make("thread-turn-stop-untargeted");
      const turnId = TurnId.make("turn-hung");

      yield* eventStore.append({
        type: "thread.created",
        eventId: EventId.make("evt-tsu1"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: now,
        commandId: CommandId.make("cmd-tsu1"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-tsu1"),
        metadata: {},
        payload: {
          threadId,
          projectId: ProjectId.make("project-turn-stop-untargeted"),
          title: "Turn stop",
          modelSelection: {
            instanceId: ProviderInstanceId.make("codex"),
            model: "gpt-5-codex",
      ...
      yield* eventStore.append({
        type: "thread.session-set",
        eventId: EventId.make("evt-tsu2"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-01-01T00:00:01.000Z",
        commandId: CommandId.make("cmd-tsu2"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-tsu2"),
        metadata: {},
        payload: {
          threadId,
          session: {
      ...
      yield* eventStore.append({
        ...
        eventId: EventId.make("evt-tsu3"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-01-01T00:05:00.000Z",
        commandId: CommandId.make("cmd-tsu3"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-tsu3"),
        metadata: {},
        payload: {
          threadId,
      ...
      `;
      assert.deepEqual(rows, [
        { turnId, state: "interrupted", completedAt: "2026-01-01T00:05:00.000Z" },
      ]);
    }),
  );
