...
function mapProjectShellRow(
  ...
  return {
    ...
    defaultBaseBranch: row.defaultBaseBranch ?? null,
    scripts: row.scripts,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const reconcileOrphanedThreads = Effect.fn("ProjectionSnapshotQuery.reconcileOrphanedThreads")(
    () =>
      Effect.gen(function* () {
        const threads = yield* sql`
      SELECT thread_id AS "threadId", worktree_path AS "worktreePath"
      FROM projection_threads
      WHERE deleted_at IS NULL AND archived_at IS NULL AND worktree_path IS NOT NULL
    `.pipe(Effect.map((rows) => rows as ReadonlyArray<Record<string, unknown>>));
        const heldRows = yield* sql`
      SELECT column_id AS "column", thread_id AS "threadId", archived_at AS "archivedAt"
      FROM kanban_cards
      WHERE archived_at IS NULL
        AND column_id IN ('active', 'pr')
        AND thread_id IS NOT NULL
    `.pipe(Effect.map((rows) => rows as ReadonlyArray<Record<string, unknown>>));
        const candidates: Array<ThreadWorktreeCandidate> = [];
        for (const row of threads) {
          const threadId = row["threadId"];
          const worktreePath = row["worktreePath"];
          if (typeof threadId !== "string" || typeof worktreePath !== "string") continue;
          const worktreeExists = yield* Effect.tryPromise(() => NodeFSP.stat(worktreePath)).pipe(
            Effect.match({ onFailure: () => false, onSuccess: () => true }),
          );
          candidates.push({ threadId, worktreePath, worktreeExists });
        }
        const heldThreadIds = liveCardThreadIds(
          heldRows.flatMap((row) => {
            ...
            return typeof column === "string" &&
              ...
              ? [{ column, threadId, archivedAt }]
              : [];
          }),
        );
        const orphanIds = orphanedThreadIds({ threads: candidates, heldThreadIds });
        if (orphanIds.length === 0) return;
        yield* sql`
      UPDATE projection_threads
      SET archived_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
      WHERE deleted_at IS NULL AND archived_at IS NULL AND thread_id IN ${sql.in(orphanIds)}
      `;
      }).pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("projection orphan cleanup skipped", { cause }),
        ),
      ),
  );
    ...
    execute: () =>
      sql`
        SELECT
          ...
          default_base_branch AS "defaultBaseBranch",
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    defaultBaseBranch: option.value.defaultBaseBranch ?? null,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      yield* reconcileOrphanedThreads();
