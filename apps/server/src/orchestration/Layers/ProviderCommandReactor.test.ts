...
const PROJECT_ROOT = "/tmp/provider-project";
const WORKTREE_ROOT = "/tmp/provider-project-worktree";

// The reactor scans the workspace for mandated skills before starting a
// session, so these roots have to be real directories, not just strings.
beforeAll(() => {
  NodeFS.mkdirSync(PROJECT_ROOT, { recursive: true });
  NodeFS.mkdirSync(WORKTREE_ROOT, { recursive: true });
});
...
describe("ProviderCommandReactor", () => {
  ...
  it("stops the session when the provider cannot deliver the interrupt", async () => {
    const harness = await createHarness({ interruptTurnFails: true });
    const now = "2026-01-01T00:00:00.000Z";
    ...
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.session?.lastError).toContain("provider is unreachable");
    expect(
      thread?.activities.find((activity) => activity.kind === "provider.turn.interrupt.failed"),
    ).toBeDefined();
  });

  it("settles a thread that has no provider session left to interrupt", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
