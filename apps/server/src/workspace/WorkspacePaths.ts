...
import { describeUnsafeWorkspaceRoot } from "./workspaceRootSafety.ts";
...
export class WorkspaceRootUnsafeError extends Schema.TaggedErrorClass<WorkspaceRootUnsafeError>()(
  "WorkspaceRootUnsafeError",
  {
    workspaceRoot: Schema.String,
  ...
  override get message(): string {
    return `Workspace root cannot be used: ${this.detail}`;
  }
}
...
export const make = Effect.gen(function* () {
  ...
  const normalizeWorkspaceRoot: WorkspacePaths["Service"]["normalizeWorkspaceRoot"] = Effect.fn(
    ...
    // An empty root resolves to the server's own cwd, so it is rejected before
    // resolution, not after.
    if (workspaceRoot.trim().length === 0) {
      return yield* new WorkspaceRootUnsafeError({
        workspaceRoot,
        detail: "the workspace root is empty",
      });
    }
    const normalizedWorkspaceRoot = path.resolve(expandHomePath(workspaceRoot.trim(), path));
    const unsafe = describeUnsafeWorkspaceRoot(normalizedWorkspaceRoot);
    if (unsafe) {
      return yield* new WorkspaceRootUnsafeError({ workspaceRoot, detail: unsafe });
    }
