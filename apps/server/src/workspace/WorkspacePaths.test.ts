...
it.layer(TestLayer)("WorkspacePathsLive", (it) => {
  describe("normalizeWorkspaceRoot", () => {
    ...
    it.effect("rejects the filesystem root and an empty root", () =>
      Effect.gen(function* () {
        const workspacePaths = yield* WorkspacePaths.WorkspacePaths;

        for (const root of ["/", "   "]) {
          const outcome = yield* Effect.result(workspacePaths.normalizeWorkspaceRoot(root));
          expect(outcome._tag).toBe("Failure");
          if (outcome._tag === "Failure") {
            expect(outcome.failure._tag).toBe("WorkspaceRootUnsafeError");
          }
        }
      }),
    );
