...
describe("buildCommitMessagePrompt", () => {
  ...
  it("carries the card's task when one is given, and omits the section when none is", () => {
    const base = { branch: "main", stagedSummary: "M a.ts", stagedPatch: "diff" } as const;

    const withMission = buildCommitMessagePrompt({
      ...base,
      includeBranch: false,
      mission: "Fix the toast\n\nMission: it never dismisses",
    });
    expect(withMission.prompt).toContain("Task this work was launched for:");
    expect(withMission.prompt).toContain("it never dismisses");

    const without = buildCommitMessagePrompt({ ...base, includeBranch: false });
    expect(without.prompt).not.toContain("Task this work was launched for:");
  });
});
...
describe("buildPrContentPrompt", () => {
  ...
  it("carries the card's task into the PR prompt", () => {
    const result = buildPrContentPrompt({
      baseBranch: "main",
      headBranch: "feature/auth",
      commitSummary: "feat: add login page",
      diffSummary: "3 files changed",
      diffPatch: "diff",
      mission: "Add a login page",
    });

    expect(result.prompt).toContain("Task this work was launched for:");
    expect(result.prompt).toContain("Add a login page");
  });
});
