...
it.layer(CursorTextGenerationTestLayer)("CursorTextGeneration", (it) => {
  ...
  it.effect("answers from the prompt's own shape when the mock derives replies", () =>
    withFakeAcpAgent({ T3_ACP_REPLY_MODE: "derive" }, (textGeneration) =>
      Effect.gen(function* () {
        const generated = yield* textGeneration.assistPrompt({
          cwd: process.cwd(),
          operation: "skill",
        ...
        expect(generated.text).toBe("make the login page dark");
      }),
    ),
  );
});
