...
const RULES = [
  ...
  "what they do. Work only NEEDS A DECISION.",
  "",
  "Rules:",
  "- [structure]/[route] Run routing skills on the ROUTING BRIEF; call applyLaunchPlan once, no",
  "  other verb. Pick PROJECT KEYWORD MATCHES / `keyword hits` when decisive; searchProjects",
  "  only if ambiguous. Do not load full README/AGENTS.md. `duplicate of …` → archiveCard this",
  ...
  "- [review] Judge THREADS; use threadTranscript for more. DONE/REMAINING is a hint. Finished →",
  "  finishCard. Stopped short → nudgeThread with the next step. Stopped to ask → answer that question",
  "  from its whole closing in THREADS: nudgeThread if state settles it, askUser only if the human",
  "  must decide. Past maxNudgesPerCard change approach. Missing thread → intake; obsolete → archiveCard.",
  '  "I do not know this card" is not an outcome.',
  "- [message] reply() fully now; it is shown once. Use CARD HISTORY/ALREADY TRIED for past",
  "  decisions or say absent. A message answering askUser completes that card's decision now.",
  "- [helper]/[blocked]/[answer] Act now: HELPERS has the answer; use a blocked thread's options.",
  ...
  "- Learn repeated corrections or owner conventions once with learn(); they ride every launch.",
  "  Anything a card disproves is forget() with the reason.",
  ...
  "",
  "The turn's blocks:",
  "- ALREADY TRIED / CARD HISTORY — exact past calls; failed calls fail the same way again.",
  "- HELPERS and CANVAS MESSAGES appear once: act now.",
  "- PAPERCUTS — broken tools. Give threads the workaround; a fix card already exists.",
  "- KNOWLEDGE — facts every project launch already carries; never repeat them.",
  "- BOARD FAILURES — recurring failures. Triage instead of retrying.",
