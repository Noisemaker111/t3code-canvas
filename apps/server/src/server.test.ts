...
import { BtwMessageRepository } from "./persistence/Services/BtwMessages.ts";
...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    btwMessages?: Partial<BtwMessageRepository["Service"]>;
    conversationLog?: Partial<ConversationLogRepository["Service"]>;
    btwChat?: Partial<BtwChatService["Service"]>;
  ...
  Effect.gen(function* () {
    ...
    const servedRoutesLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      // The btw side-chat services are constructed by `makeWsRpcLayer`, so the
      // /ws route dies with "Service not found: BtwMessageRepository" — and a
      // dead route is a 500 on the upgrade, which is how every websocket test
      // in this file used to fail. Stubbed here because none of these tests
      // exercise btw itself. Merged into the keybindings `provide` rather than
      // taking slots of their own: this `pipe` is at its 20-argument limit.
      Layer.provide(
        Layer.mergeAll(
          // The maintenance routes survey history straight off SQL, so without a
          // client every route in this layer fails to build. Same reason as btw
          // below, same slot: this `pipe` is at its 20-argument limit.
          SqlitePersistenceMemory,
          Layer.mock(BtwMessageRepository)({
            append: () => Effect.void,
            listByThreadId: () => Effect.succeed([]),
            deleteByThreadId: () => Effect.void,
            ...options?.layers?.btwMessages,
          }),
          Layer.mock(ConversationLogRepository)({
            listByThreadId: () => Effect.succeed([]),
            ...options?.layers?.conversationLog,
          }),
          Layer.mock(BtwChatService)({
            streamReply: () => Stream.empty,
            ...options?.layers?.btwChat,
          }),
      ...
      Layer.provide(
        Layer.mergeAll(
          Layer.mock(KanbanStore.KanbanStore)({
            list: () => Effect.succeed({ cards: [] }),
            create: () => Effect.die("KanbanStore.create is not mocked"),
            update: () => Effect.die("KanbanStore.update is not mocked"),
            remove: (input) => Effect.succeed({ id: input.id, deleted: false }),
          }),
          Layer.mock(CanvasStore.CanvasStore)({
            get: () => Effect.die("CanvasStore.get is not mocked"),
            save: () => Effect.die("CanvasStore.save is not mocked"),
            listInjections: () => Effect.succeed({ injections: [] }),
            ackInjections: () => Effect.succeed({ acked: 0 }),
            digest: () => Effect.die("CanvasStore.digest is not mocked"),
            enqueueInjection: () => Effect.die("CanvasStore.enqueueInjection is not mocked"),
          }),
          Layer.mock(TextGeneration)({
            generateCommitMessage: () => Effect.die("generateCommitMessage is not mocked"),
            generatePrContent: () => Effect.die("generatePrContent is not mocked"),
            generateBranchName: () => Effect.die("generateBranchName is not mocked"),
            generateThreadTitle: () => Effect.die("generateThreadTitle is not mocked"),
            assistPrompt: () => Effect.die("assistPrompt is not mocked"),
          }),
          Layer.mock(UpdateService.UpdateService)({
            ...
            checkForUpdate: Effect.die("checkForUpdate is not mocked"),
            installUpdate: () => Effect.die("installUpdate is not mocked"),
          }),
          Layer.mock(VpsHostService.VpsHost)({
            snapshot: () => Effect.die("VpsHost.snapshot is not mocked"),
            serviceAction: () => Effect.die("VpsHost.serviceAction is not mocked"),
            signalProcess: () => Effect.die("VpsHost.signalProcess is not mocked"),
          }),
      ...
      Layer.provide(
        Layer.mergeAll(
          Layer.mock(AgentBrowserManager.AgentBrowserManager)({
            subscribeSessions: () => Effect.succeed(() => undefined),
          }),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("reports workspace root stat failures without relabeling them as missing", () =>
    Effect.gen(function* () {
      ...
      // A 0o000 directory does not block root, so the stat this test needs to
      // fail simply succeeds and the RPC never errors. T3J runs as root by
      // design, so this is a normal environment, not a broken one.
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          baseRefName: "main",
          // Thread-scoped and absolute, not `null`. Passing null let rift pick
          // its own root; keying on the thread keeps two threads on one base
          // branch out of each other's directory.
