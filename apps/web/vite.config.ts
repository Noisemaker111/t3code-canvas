...
// vps-code deploy stamp: the outer repo commit and build time are injected by
// deploy/apply.sh so the running site can show exactly which release is live
// (i.e. when the last update landed). Kept separate from APP_VERSION so it does
// not interfere with client/server semver skew checks.
const configuredBuildSha = process.env.T3CODE_BUILD_SHA?.trim() || "dev";
const configuredBuildTime = process.env.T3CODE_BUILD_TIME?.trim() || new Date().toISOString();
  ...
  return {
    ...
    optimizeDeps: {
      include: [
        ...
        "effect/Order",
        "react-dom/client",
        // tldraw pulls CJS-only deps (lodash.isequalwith, classnames);
        // native ESM cannot read their default export, so it has to be
        // pre-bundled. Resolving its undeclared @tldraw/state import is
        // why apps/web depends on that package directly.
        "tldraw",
        "@tldraw/state",
      ],
      // @tldraw/assets/imports.vite imports every icon and translation
      // with `?url`, a Vite transform the dependency optimizer does not
      // run — pre-bundling it fails to load all 53 translations.
      // onnxruntime-web ships its own WASM loader; pre-bundling breaks wasmPaths.
      exclude: ["@tldraw/assets/imports.vite", "onnxruntime-web", "onnx-asr-web"],
    },
    define: {
      ...
      "import.meta.env.T3CODE_BUILD_SHA": JSON.stringify(configuredBuildSha),
