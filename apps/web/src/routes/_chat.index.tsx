...
/**
 * Index always lands on the Board. Empty "New thread" draft landing is gone —
 * capture goes through the board composer (Hermes).
 */
function ChatIndexRouteView() {
  ...
  useEffect(() => {
    if (authGateState.status === "hosted-static" && environments.length === 0) return;
    void navigate({ to: "/kanban", replace: true });
  }, [authGateState.status, environments.length, navigate]);
