...
describe("shouldSubmitComposerOnEnter", () => {
  ...
  it("submits Ctrl/Cmd+Enter even on mobile", () => {
    expect(
      shouldSubmitComposerOnEnter({
  ...
  it("does not submit Ctrl+Shift+Enter", () => {
    expect(
      shouldSubmitComposerOnEnter({
...
describe("expandLeadingComposerSkillCommand", () => {
  const prompts = new Map([["handoff", "Summarize where we left off and what's next."]]);

  it("expands a bare command", () => {
    expect(expandLeadingComposerSkillCommand("/handoff", prompts)).toBe(
      "Summarize where we left off and what's next.",
    );
  });

  it("expands a command with trailing text, preserving it verbatim", () => {
    expect(expandLeadingComposerSkillCommand("/handoff also mention the flaky test", prompts)).toBe(
      "Summarize where we left off and what's next. also mention the flaky test",
    );
  });
  ...
  it("leaves an unregistered command untouched", () => {
    expect(expandLeadingComposerSkillCommand("/unknown-command", prompts)).toBe("/unknown-command");
  });

  it("leaves non-command text untouched", () => {
    expect(expandLeadingComposerSkillCommand("just a normal message", prompts)).toBe(
      "just a normal message",
    );
  });

  it("does not partial-match a command name that is a prefix of another word", () => {
    expect(expandLeadingComposerSkillCommand("/handofftypo", prompts)).toBe("/handofftypo");
  });
});
