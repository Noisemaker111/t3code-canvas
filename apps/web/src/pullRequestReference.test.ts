...
import { parsePullRequestNumber, parsePullRequestReference } from "./pullRequestReference";
...
describe("parsePullRequestNumber", () => {
  it("reads the number off a forge url", () => {
    expect(parsePullRequestNumber("https://github.com/Noisemaker111/vps-code/pull/1450")).toBe(
      1450,
    );
    expect(parsePullRequestNumber("https://gitlab.com/group/app/-/merge_requests/7")).toBe(7);
  });

  it("has nothing to show for a branch or an empty card", () => {
    expect(parsePullRequestNumber("feature/my-branch")).toBeNull();
