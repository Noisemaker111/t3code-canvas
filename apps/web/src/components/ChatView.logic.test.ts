...
describe("shouldRenderBranchToolbar", () => {
  it("shows the toolbar for a repository", () => {
    expect(shouldRenderBranchToolbar({ isGitRepo: true, branch: null, worktreePath: null })).toBe(
      true,
    );
  });

  it("keeps metadata-backed threads visible when the status probe is not a repo", () => {
    expect(
      shouldRenderBranchToolbar({
        isGitRepo: false,
  ...
  it("does not add the toolbar to a non-repository thread without git metadata", () => {
    expect(shouldRenderBranchToolbar({ isGitRepo: false, branch: null, worktreePath: null })).toBe(
      false,
    );
  });
});
...
describe("shouldTypeToFocusComposer", () => {
  ...
  const keyOn = (element: FakeElement) =>
    ({
      key: "r",
  ...
  it("types into the composer from an ordinary surface", () => {
    stub();
    expect(shouldTypeToFocusComposer(keyOn(new FakeElement(["div"])))).toBe(true);
  });

  it("leaves the key to tldraw while the canvas owns the keyboard", () => {
    stub();
    expect(shouldTypeToFocusComposer(keyOn(new FakeElement([".tl-container__focused"])))).toBe(
      false,
    );
  });
});
