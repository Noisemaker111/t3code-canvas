...
const ENDPOINT_ROW_CLASSNAME = "border-t border-border/60 px-4 py-3.5 first:border-t-0 sm:px-5";
          ...
          <Dialog open={isRevealDialogOpen} onOpenChange={setIsRevealDialogOpen}>
            ...
            <DialogPopup className="max-w-md">
              ...
              <DialogPanel className="space-y-4">
                ...
                />
                {shareablePairingUrl ? (
                  <div className="flex justify-center rounded-xl bg-raised p-4">
                    <QRCodeSvg
                      value={shareablePairingUrl}
                      size={132}
                      level="M"
        ...
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex min-h-5 items-center gap-1.5">
            ...
            {clientSession.current ? (
              <span className="text-[10px] text-muted-foreground/80 rounded-md bg-raised px-1 py-0.5">
                This device
              </span>
            ) : null}
          </div>
...
function CloudRemoteEnvironmentRows({
  ...
  return hasCloudPublicConfig() ? (
    ...
    />
  ) : // Cloud off + nothing saved: no permanent empty box — the header's Add
  // button is the whole affordance.
  null;
}

export function ConnectionsSettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const renderSshFields = () => (
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-lg border border-border/60">
        <div className="flex items-center justify-between gap-3 border-b border-border/60 bg-muted/30 px-4 py-2 sm:px-5">
  ...
  const renderEndpointRows = (presentation: AccessSectionPresentation) =>
    isAdvertisedEndpointListExpanded
      ...
      : null;
  // Apply a setting change immediately. The WSL reconciler updates the
  ...
  // side persists the change and nudges its reconciler, we trigger
  ...
  const renderDisabledNetworkAccessRow = () => (
    <SettingsRow
      ...
      description={
        currentAuthPolicy === "remote-reachable"
          ? "This backend is configured for remote access. Network exposure changes must be made where the server is launched."
          : "This backend listens on loopback only. Remote access, when present, is provided by a tunnel or reverse proxy in front of it (e.g. Cloudflare Access) — binding other interfaces would bypass it."
      }
      control={
        <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">
          {currentAuthPolicy === "remote-reachable" ? "Remote" : "Loopback"}
        </span>
      }
    />
  );
  ...
  return settingsPageShell(
    ...
    <>
      {canManageLocalBackend ? (
        <>
          <SettingsSection title="Access · This environment">
      ...
      <SettingsSection
        title="Access · Remote environments"
        headerAction={
          <Dialog
