...
/**
 * Who the board commits as.
 *
 * The board owns git — the agent never commits — so every commit it makes
 * carries the box's git identity. A VPS with no `user.email` configured lets
 * git invent one from the login and hostname, which is why merged pull
 * requests pick up `Co-authored-by: root <root@vps-….ovh.us>`. Setting both
 * fields here stamps `-c user.name`/`-c user.email` on every board commit.
 *
 * The avatar is not ours to set: GitHub resolves it from the author email, so
 * the one click that gets both the picture and the name right is adopting the
 * signed-in `gh` account, whose noreply address nobody should have to type.
 */
function CommitIdentitySection() {
  const environmentId = usePrimaryEnvironment()?.environmentId ?? null;
  const savedName = usePrimarySettings((settings) => settings.commitAuthorName);
  const savedEmail = usePrimarySettings((settings) => settings.commitAuthorEmail);
  const updateSettings = useUpdatePrimarySettings();
  const [nameDraft, setNameDraft] = useState(savedName);
  const [emailDraft, setEmailDraft] = useState(savedEmail);

  useEffect(() => setNameDraft(savedName), [savedName]);
  useEffect(() => setEmailDraft(savedEmail), [savedEmail]);

  const accountIdentity = useEnvironmentQuery(
    environmentId === null
      ? null
      : sourceControlEnvironment.accountIdentity({ environmentId, input: {} }),
  );
  const account = accountIdentity.data ?? null;
  const [isAvatarBroken, setIsAvatarBroken] = useState(false);
  const nextName = nameDraft.trim();
  const nextEmail = emailDraft.trim();
  const isDirty = nextName !== savedName || nextEmail !== savedEmail;
  const isHalfSet = nextName.length > 0 !== nextEmail.length > 0;
  const isAccountEmail = account !== null && savedEmail === account.email;
  const avatarUrl = isAccountEmail ? optionLabel(account.avatarUrl) : null;
  const status = isHalfSet
    ? "Both fields are needed. With one empty the box's git config still decides, and an unconfigured box commits as root@hostname."
    : savedName.length > 0
      ? isAccountEmail
        ? `Commits show as ${savedName} on @${account.account}.`
        : `Board commits are authored ${savedName} <${savedEmail}>.`
      : "Unset — commits use the box's own git config. On a VPS with no user.email that resolves to root@hostname and lands as the co-author of every squash-merged PR.";

  return (
    <SettingsSection title="Commit identity">
      <SettingsRow
        title="Author"
        description={status}
        control={
          <div className="flex flex-col items-end gap-2">
            {account !== null && !isAccountEmail ? (
              <Button
                size="xs"
                variant="outline"
                aria-label={`Commit as GitHub account ${account.account}`}
                onClick={() =>
                  updateSettings({
                    commitAuthorName: account.name,
                    commitAuthorEmail: account.email,
                  })
                }
              >
            ...
            {avatarUrl !== null && !isAvatarBroken ? (
              <img
                src={avatarUrl}
                alt=""
                className="size-8 rounded-full bg-raised"
            ...
            <input
              ...
              placeholder="1234567+you@users.noreply.github.com"
              className="h-8 w-64 rounded-md bg-raised px-2 font-mono text-xs outline-none focus:border-ring"
              value={emailDraft}
            ...
            <Button
              size="xs"
              variant="outline"
              aria-label="Save commit identity"
              disabled={!isDirty || isHalfSet}
              onClick={() =>
                updateSettings({ commitAuthorName: nextName, commitAuthorEmail: nextEmail })
              }
            >
...
export function SourceControlSettingsPanel({ embedded = false }: { embedded?: boolean } = {}) {
  const environmentId = usePrimaryEnvironment()?.environmentId ?? null;
