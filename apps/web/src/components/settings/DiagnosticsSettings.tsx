...
import { SettingsSection, useRelativeTimeTick, settingsPageShell } from "./settingsLayout";
...
/**
 * The incident log (`recordDegradation` + failed health checks) — the surface
 * the no-silent-fallback rule points degradations at. One row per check id,
 * newest first, with the "seen N× today" counter.
 */
function HealthIncidentsSection() {
  const [incidents, setIncidents] = useState<ReadonlyArray<HealthIncident> | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const body = (await response.json()) as { incidents?: ReadonlyArray<HealthIncident> };
  ...
  const rows = useMemo(() => {
    ...
    const byId = new Map<string, { latest: HealthIncident; total: number; today: number }>();
    // Server returns newest first, so the first entry per id is the latest.
    for (const incident of incidents) {
      const at = Date.parse(incident.at);
      const existing = byId.get(incident.id);
      const today = Number.isFinite(at) && at >= dayFloor ? 1 : 0;
      if (existing) {
        existing.total += 1;
        existing.today += today;
      } else {
        byId.set(incident.id, { latest: incident, total: 1, today });
      }
    }
    return [...byId.values()];
  }, [incidents]);
    ...
    >
      {error ? (
        <div className="flex items-start gap-2 px-4 py-3 text-xs text-destructive sm:px-5">
          ...
          <span>Could not read the incident log — {error}</span>
        </div>
      ) : incidents === null ? (
        ...
        <p className="px-4 py-3 text-xs text-muted-foreground sm:px-5">
          No recorded incidents. Degradations and failed health checks land here.
        </p>
      ) : (
        <div className="divide-y divide-border/60">
          {rows.map((row) => (
            <div key={row.latest.id} className="px-4 py-2.5 sm:px-5">
              <div className="flex flex-wrap items-baseline gap-x-2 text-xs">
                <code className="font-medium text-foreground">{row.latest.id}</code>
                <span
                  className={cn(
                    "rounded-full px-1.5 py-0.5 text-[10px] font-medium",
                    row.latest.severity === "critical"
                ...
                {row.today > 0 ? (
                  <span className="text-[11px] text-muted-foreground">seen {row.today}× today</span>
                ) : null}
                <span className="ml-auto text-[11px] tabular-nums text-muted-foreground">
                  {new Date(row.latest.at).toLocaleString([], {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                    minute: "2-digit",
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">{row.latest.detail}</p>
              {row.latest.fixed !== undefined ? (
                <p className="mt-0.5 text-[11px] text-muted-foreground">
                  {row.latest.fixed
                    ? "Auto-fix succeeded."
                    : `Auto-fix failed${row.latest.fixError ? `: ${row.latest.fixError}` : "."}`}
                </p>
              ) : null}
            </div>
          ))}
...
export function DiagnosticsSettingsPanel({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const openLogsDirectory = useCallback(() => {
    ...
    if (!editor) {
      // Headless box: nothing can open a folder — hand over the path instead.
      void (async () => {
        try {
          await navigator.clipboard.writeText(logsDirectoryPath);
          setOpenLogsDirectoryError(`No editor on this host — path copied: ${logsDirectoryPath}`);
        } catch {
          setOpenLogsDirectoryError(`Logs folder: ${logsDirectoryPath}`);
        }
      })();
      return;
    }
