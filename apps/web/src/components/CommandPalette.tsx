...
import { addFrameEntries, addPanelEntries } from "./canvas/panels/addMenu";
...
function OpenCommandPaletteDialog(props: {
  ...
  const defaultModelSelection = usePrimarySettings((settings) => settings.defaultModelSelection);
  ...
  const openProjectFromSearch = useMemo(
    () => async (_project: (typeof projects)[number]) => {
      // Board-first: project search opens the kanban, not an empty "New thread" draft.
  ...
  // Putting a closed page back on the canvas. The request is left in the
  // station store and the canvas materializes it — the board is home, so this
  // lands on `/kanban` with the panel on it rather than on a page of its own.
  for (const entry of addPanelEntries()) {
    actionItems.push({
      kind: "action",
      value: `action:add-panel:${entry.kind}`,
      searchTerms: [`add ${entry.label}`, entry.label, "panel", "component", "canvas"],
      title: `Add ${entry.label} panel`,
      icon: <LayoutDashboardIcon className={ITEM_ICON_CLASS} />,
      run: async () => {
        requestPanel({ kind: entry.kind, entityId: "" }, { focus: false });
        await navigate({ to: "/kanban" });
      },
    });
  }

  // Dropping in a frame. The same list the canvas menu offers, so every frame
  // this build ships — the laid-out ones included — is one action from the
  // keyboard rather than three of them being right-click only.
  for (const entry of addFrameEntries()) {
    const phone = entry.preset === "mobile";
    actionItems.push({
      kind: "action",
      value: `action:add-frame:${entry.preset}`,
      searchTerms: [
        `add ${entry.label} frame`,
        entry.label,
        "frame",
        "screen",
        "layout",
      ...
      title: `Add ${entry.label.toLowerCase()} frame`,
      description: entry.detail,
      icon: phone ? (
        <SmartphoneIcon className={ITEM_ICON_CLASS} />
      ) : (
        <MonitorIcon className={ITEM_ICON_CLASS} />
      ),
      run: async () => {
        requestFrame(entry.preset, { focus: true });
        await navigate({ to: "/kanban" });
      },
    });
  }
  ...
  actionItems.push({
    ...
    run: async () => {
      await navigate({ to: "/kanban", search: { station: "settings" } });
    },
  });
    ...
    async (input: {
      ...
      toastManager.add(
        stackedThreadToast({
          type: "success",
          title: "Project added",
          description: "Use the Board composer — no empty New thread page.",
        }),
      );
      setOpen(false);
    },
    [createProject, navigate, projects, setOpen],
  );
