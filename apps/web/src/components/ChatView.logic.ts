...
export function shouldRenderBranchToolbar(input: {
  ...
  return input.isGitRepo || input.branch != null || input.worktreePath != null;
}
...
// A thread is a panel on the canvas and this rule runs off a window listener:
// without it every letter aimed at the canvas landed in the transcript's
// composer and tldraw's tool shortcuts (r, o, t, d …) never fired. The class is
// tldraw's own "the canvas owns the keyboard" flag, which the panel layer clears
// the moment a panel is hovered, focused or opened as a station — so typing into
// an open thread still reaches the composer.
const TYPE_TO_FOCUS_CANVAS_SELECTOR = ".tl-container__focused";
...
/** Whether a bare keystroke should be typed into the thread composer. */
export function shouldTypeToFocusComposer(event: KeyboardEvent): boolean {
  ...
  if (eventPathContainsSelector(event, TYPE_TO_FOCUS_CANVAS_SELECTOR)) return false;
