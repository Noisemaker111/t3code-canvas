...
import { routeFactsKey, useModelRouteFacts } from "../../lib/modelRouteFacts";
...
export function autoOptionMatchesSearch(option: AutoModelOption, query: string): boolean {
...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  /**
   * Pinned non-model row above the list (board composer / card editor: let the
   * router pick instead of a fixed model). Absent in thread contexts.
   */
  ...
  const routeFacts = useModelRouteFacts();
  ...
  const usageBlockedByInstance = useMemo(() => {
    const blocked = new Map<ProviderInstanceId, string>();
    for (const entry of instanceEntries) {
      const reason = providerUsageUnusableReason(usageSnapshot.data, entry.instanceId);
      if (reason) blocked.set(entry.instanceId, reason);
    }
    return blocked;
  }, [instanceEntries, usageSnapshot.data]);

  const readyInstanceSet = useMemo(() => {
    const ready = new Set<ProviderInstanceId>();
    for (const entry of instanceEntries) {
      if (isProviderInstancePickerReady(entry) && !usageBlockedByInstance.has(entry.instanceId)) {
        ready.add(entry.instanceId);
      }
    }
    return ready;
  }, [instanceEntries, usageBlockedByInstance]);
  ...
  const disabledInstanceIds = useMemo(() => {
    const disabled = new Set<ProviderInstanceId>(usageBlockedByInstance.keys());
    if (lockedDisabledInstanceIds) {
      for (const id of lockedDisabledInstanceIds) disabled.add(id);
    }
    return disabled.size > 0 ? disabled : undefined;
  }, [lockedDisabledInstanceIds, usageBlockedByInstance]);
  ...
  const sectionedListRows = useMemo(
    (): ReadonlyArray<ModelPickerListRow<ModelPickerItem>> =>
      buildModelPickerSectionedRows(filteredModels),
    [filteredModels],
  );
  const listShowsSections = useMemo(
    () => modelPickerListShowsSections(sectionedListRows),
    [sectionedListRows],
  );
  const listRowKeys = useMemo(() => sectionedListRows.map((row) => row.id), [sectionedListRows]);
  const listRowByKey = useMemo(
    () => new Map(sectionedListRows.map((row) => [row.id, row] as const)),
    [sectionedListRows],
  );
  /** Combobox model index → virtual list row index (accounts for section headers). */
  const listIndexByComboboxIndex = useMemo(() => {
    const map = new Map<number, number>();
    sectionedListRows.forEach((row, listIndex) => {
      if (row.kind === "model") {
        map.set(row.comboboxIndex, listIndex);
      }
    });
    return map;
  }, [sectionedListRows]);
  ...
  useLayoutEffect(() => {
    ...
    setShowBottomScrollFade(listRowKeys.length > 5);
    let nestedFrame = 0;
    const frame = window.requestAnimationFrame(() => {
  ...
  }, [listRowKeys, updateModelListScrollFades]);
      ...
      >
            ...
            {...(disabledInstanceIds
              ? {
                  ...
                  getDisabledInstanceTooltip: (entry: ProviderInstanceEntry) => {
                    const usageReason = usageBlockedByInstance.get(entry.instanceId);
                    if (usageReason) {
                      return `${entry.displayName} — ${usageReason}`;
                    }
        ...
        <Combobox
          ...
          onItemHighlighted={(modelKey, eventDetails) => {
            ...
            if (eventDetails.reason === "keyboard" && eventDetails.index >= 0) {
              const listIndex =
                listIndexByComboboxIndex.get(eventDetails.index) ?? eventDetails.index;
        ...
        >
          ...
          >
            ...
            {props.autoOption && autoOptionMatchesSearch(props.autoOption, searchQuery) ? (
              <button
                type="button"
                onClick={() => {
                  props.autoOption?.onSelect();
                  props.onRequestClose?.();
                }}
                className={cn(
                  "mx-1 mt-2 flex flex-col items-start gap-0.5 rounded-md px-2 py-2 text-left transition-colors",
                  props.autoOption.selected
                    ? "bg-accent text-foreground"
                    : "hover:bg-accent/60 text-foreground/90",
                )}
              >
                <span className="flex items-center gap-1.5 text-sm">
                  <ShuffleIcon className="size-3.5 shrink-0 text-muted-foreground" aria-hidden />
                  {props.autoOption.label}
                </span>
                {props.autoOption.description ? (
            ...
            {/* Model list — sectioned by harness / upstream router when mixed */}
            <div className="relative min-h-0 flex-1 overflow-hidden">
              <ComboboxListVirtualized className="model-picker-list size-full min-w-0 p-0">
                <LegendList<string>
                  ...
                  getItemType={(rowKey) =>
                    listRowByKey.get(rowKey)?.kind === "section" ? "section" : "model"
                  }
                  getFixedItemSize={(rowKey) => {
                    const row = listRowByKey.get(rowKey);
                    if (row?.kind === "section") return 28;
                    return listShowsSections ? 44 : 60;
                  }}
                  renderItem={({ item: rowKey }) => {
                    ...
                    if (row.kind === "section") {
                      return (
                        <div
                          key={row.id}
                          className="bg-muted/50 px-3 pb-1 pt-2"
                          data-model-picker-section={row.label}
                        >
                    ...
                    return (
                      <ModelListRow
                        ...
                        // Section header already names the provider/router.
                        showProvider={!listShowsSections}
                        preferShortName={!isLocked}
                        useTriggerLabel={false}
                        showNewBadge={isModelPickerNewModel(model.driverKind, model.slug)}
                        disabledReason={disabledReason}
                        facts={routeFacts.get(routeFactsKey(model.instanceId, model.slug))}
                        onToggleFavorite={() => toggleFavorite(model.instanceId, model.slug)}
                      />
                    );
                  }}
                  estimatedItemSize={listShowsSections ? 44 : 60}
                  drawDistance={480}
                  recycleItems
