...
function groupCommandItems(
  ...
  const globalSkillCommandItems = items.filter((item) => item.type === "global-skill-command");
  ...
  if (globalSkillCommandItems.length > 0) {
    groups.push({ id: "global-skill", label: "Global Skills", items: globalSkillCommandItems });
  }
...
const ComposerCommandMenuItem = memo(function ComposerCommandMenuItem(props: {
  ...
  return (
    ...
    >
      ...
      {props.item.type === "global-skill-command" ? (
        <BotIcon className="size-4 shrink-0 text-muted-foreground/80" />
      ) : null}
