...
/** Compact "4s · 420 tok" parts for a response/tool metrics badge. */
function formatRowMetricsLabel(metrics: MessageRowMetrics): string {
  const parts: string[] = [];
  if (metrics.durationMs !== undefined) {
    parts.push(formatDuration(metrics.durationMs));
  }
  if (metrics.tokens !== undefined) {
    parts.push(`${formatContextWindowTokens(metrics.tokens)} tok`);
  }
  return parts.join(" · ");
}
...
function AssistantTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  const metricsLabel = row.metrics ? formatRowMetricsLabel(row.metrics) : "";

  return (
    <>
      <div className="relative min-w-0 px-1 py-0.5">
        {metricsLabel ? (
          <div className="mb-1 select-none text-[11px] tabular-nums text-muted-foreground/55">
            {metricsLabel}
          </div>
        ) : null}
        <ChatMarkdown
  ...
  return (
    <div className="py-0.5 pl-1.5">
      <div className="flex items-center gap-2 pt-1 text-[11px] text-muted-foreground/70 tabular-nums">
        <AtomSpinner size={16} label="Working" />
        <span>
          {row.createdAt ? (
            <>
      ...
      <div className="flex select-none items-center gap-1.5 transition-[opacity,translate] duration-200">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5">
          ...
          <div className="flex shrink-0 items-center gap-px text-muted-foreground/55">
            {workEntry.tokenCount ? (
              <span className="mr-1 shrink-0 text-[11px] tabular-nums text-muted-foreground/55">
                {formatContextWindowTokens(workEntry.tokenCount)} tok
              </span>
            ) : null}
            {workEntry.durationMs !== undefined ? (
              <span className="mr-1 shrink-0 text-[11px] tabular-nums text-muted-foreground/50">
