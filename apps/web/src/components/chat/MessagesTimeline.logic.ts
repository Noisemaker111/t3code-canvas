...
import { type MessageMetrics, type TurnMetrics } from "../../lib/contextWindow";
...
export type MessagesTimelineRow =
  ...
  | {
      ...
      metrics?: MessageRowMetrics | undefined;
    }
  | {
      kind: "proposed-plan";
      id: string;
...
/** Minimal per-message readout: the tokens and time a response turn used. */
...
/**
 * Resolve the tokens/time to show on a terminal assistant response. Tokens
 * prefer the message's own server-counted metrics, then the turn's reported
 * usage; duration prefers the harness-reported turn duration and otherwise
 * falls back to the span from the turn's start boundary to when this message
 * finished. Returns undefined when nothing is known.
 */
export function resolveAssistantRowMetrics(input: {
  turnId: TurnId | null;
  messageId?: string;
  durationStart: string;
  updatedAt: string;
  streaming: boolean;
  turnMetricsByTurnId: ReadonlyMap<string, TurnMetrics> | undefined;
  messageMetricsByMessageId?: ReadonlyMap<string, MessageMetrics> | undefined;
}): MessageRowMetrics | undefined {
  const turnMetrics =
    input.turnId != null ? input.turnMetricsByTurnId?.get(input.turnId) : undefined;
  const messageMetrics =
    input.messageId != null ? input.messageMetricsByMessageId?.get(input.messageId) : undefined;
  const tokens =
    messageMetrics !== undefined && messageMetrics.tokens > 0
      ? messageMetrics.tokens
      : turnMetrics?.outputTokens != null && turnMetrics.outputTokens > 0
        ? turnMetrics.outputTokens
        : undefined;

  let durationMs: number | undefined;
  if (turnMetrics?.durationMs != null && turnMetrics.durationMs > 0) {
    durationMs = turnMetrics.durationMs;
  } else if (!input.streaming) {
    const started = Date.parse(input.durationStart);
    const ended = Date.parse(input.updatedAt);
...
export function deriveMessagesTimelineRows(input: {
  ...
  turnMetricsByTurnId?: ReadonlyMap<string, TurnMetrics>;
  messageMetricsByMessageId?: ReadonlyMap<string, MessageMetrics>;
}): MessagesTimelineRow[] {
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      ...
      const visibleGroupedEntries = groupedEntries.filter(
        (entry) =>
          !workEntryIndicatesToolNeutralStatus(entry) && !workEntryIsProviderRetryWarning(entry),
      );
    ...
    nextRows.push({
      ...
      metrics:
        timelineEntry.message.role === "assistant" && showAssistantMeta
          ? resolveAssistantRowMetrics({
              turnId: timelineEntry.message.turnId,
              messageId: timelineEntry.message.id,
              durationStart,
              updatedAt: timelineEntry.message.updatedAt,
              streaming: timelineEntry.message.streaming,
              turnMetricsByTurnId: input.turnMetricsByTurnId,
              messageMetricsByMessageId: input.messageMetricsByMessageId,
            })
          : undefined,
    });
  }
