...
/**
 * LegendList `scrollToEnd` aligns the last row to the viewport bottom and only
 * subtracts style padding / footer — not `contentInsetEndAdjustment`. Pass this
 * as `viewOffset` so live-follow and the scroll-to-end control clear the docked
 * composer overlay for every provider.
 */
export function resolveComposerScrollToEndViewOffset(composerOverlayHeight: number): number {
  if (!Number.isFinite(composerOverlayHeight) || composerOverlayHeight <= 0) {
    return 0;
  }
  return -Math.ceil(composerOverlayHeight);
}
