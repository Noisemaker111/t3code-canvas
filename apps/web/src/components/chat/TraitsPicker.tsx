...
function ReasoningBattery({
  ...
  return (
    <span className="inline-flex items-center gap-1.5" title={`Reasoning: ${label}`}>
      <span className="relative inline-flex h-4 w-2.5 flex-col justify-end gap-px rounded-[2px] border border-current p-px text-current opacity-80">
        <span className="absolute -top-1 left-1/2 h-0.5 w-1 -translate-x-1/2 rounded-t bg-current" />
        {Array.from({ length: levels }, (_, index) => (
          <span
            key={index}
            className={cn(
              "min-h-0.5 flex-1 rounded-[1px]",
              index < level ? "bg-current" : "bg-current/15",
            )}
          />
        ))}
      </span>
...
export const TraitsPicker = memo(function TraitsPicker({
  ...
  const reasoningDescriptor =
    primarySelectDescriptor?.id === "effort" ? primarySelectDescriptor : null;
  const reasoningValue = reasoningDescriptor ? getDescriptorStringValue(reasoningDescriptor) : null;
  const reasoningIndex = reasoningDescriptor
    ? Math.max(
        0,
        reasoningDescriptor.options.findIndex((option) => option.id === reasoningValue),
      ) + 1
    : 0;
      ...
      >
        {reasoningDescriptor ? (
          <span className="flex min-w-0 items-center gap-1.5 overflow-hidden">
            <ReasoningBattery
              level={reasoningIndex}
              levels={reasoningDescriptor.options.length}
