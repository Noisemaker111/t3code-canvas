...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  const {
    ...
    showPlanFollowUpPrompt: _showPlanFollowUpPrompt,
    activeProposedPlan,
    activePlan,
    sidebarProposedPlan,
    planSidebarLabel,
  ...
  // Chat sessions always run with unrestricted access; the mode controls are intentionally hidden.
    ...
    if (composerTrigger.kind === "slash-command") {
      const builtInSlashCommandItems = [
        ...
        },
        {
          ...
          command: "btw",
          label: "/btw",
          description: "Open a side chat about this conversation",
        },
      ] satisfies ReadonlyArray<Extract<ComposerCommandItem, { type: "slash-command" }>>;
      const globalSkillCommandItems = Object.entries(settings.skillCommands).map(
        ([id, command]) => ({
          id: `global-skill-command:${id}`,
          type: "global-skill-command" as const,
          skillCommandId: id as SkillCommandId,
    ...
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "slash-command") {
        ...
        if (item.command === "btw") {
          ...
          if (applied) {
            setComposerHighlightedItemId(null);
            useBtwPanelStore.getState().open(routeThreadRef);
          }
          return;
        }
        return;
      ...
      if (item.type === "global-skill-command") {
        // Replace the `/name` token with the full global skill text so the
        // composer shows what will be sent (not a deferred expand-on-send token).
        const skillPrompt = item.prompt.trim() || `/${item.skillCommandId}`;
        const replacement = skillPrompt.endsWith("\n") ? skillPrompt : `${skillPrompt}\n`;
  ...
  const onComposerPaste = (event: React.ClipboardEvent<HTMLElement>) => {
    ...
    const pastedImages = Array.from(event.clipboardData.items)
      ...
      .map((item, index) => {
        ...
        return blob
          ? new File([blob], `pasted-image-${index + 1}.${blob.type.split("/")[1] ?? "png"}`, {
              type: blob.type,
            })
          : null;
      })
            ...
            >
              <div className="-m-1 flex min-w-0 flex-1 items-center gap-2 overflow-x-auto p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <ProviderModelPicker
                  ...
                  reasoningControl={providerTraitsPicker}
