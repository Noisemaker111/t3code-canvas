...
describe("resolveAssistantRowMetrics", () => {
  const base = {
    turnId: "turn-1" as never,
  ...
  it("prefers the message's own server-counted tokens over turn usage", () => {
    const metrics = resolveAssistantRowMetrics({
      ...base,
      turnMetricsByTurnId: new Map([["turn-1", { outputTokens: 9_999, durationMs: 4_000 }]]),
      messageMetricsByMessageId: new Map([["assistant:item-1", { tokens: 210 }]]),
    });

    expect(metrics).toEqual({ tokens: 210, durationMs: 4_000 });
  });

  it("falls back to turn usage, then to the message's elapsed span", () => {
    const metrics = resolveAssistantRowMetrics({
      ...base,
      turnMetricsByTurnId: new Map([["turn-1", { outputTokens: 350, durationMs: null }]]),
      messageMetricsByMessageId: new Map(),
    });

    expect(metrics).toEqual({ tokens: 350, durationMs: 12_000 });
  });
});
