...
export function ContextWindowMeter(props: {
  ...
  /** When set, click activates this (e.g. open the thread) instead of only the hover popover. */
  onClick?: () => void;
}) {
  const { usage, providerDisplayName, onClick } = props;
      ...
      <PopoverTrigger
        ...
        render={
          <button
            ...
            aria-label={onClick ? `${usageLabel}. Open thread.` : usageLabel}
            title={onClick ? "Open thread" : undefined}
            onClick={(event) => {
              if (!onClick) return;
