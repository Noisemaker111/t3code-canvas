...
import { ProviderDriverKind, SkillCommandId } from "@t3tools/contracts";
...
describe("searchSlashCommandItems", () => {
  ...
  it("matches global skill commands by id and ranks an exact match first", () => {
    const items = [
      {
        id: "provider-slash-command:claudeAgent:handoff-notes",
        type: "provider-slash-command",
        provider: claudeDriver,
        command: { name: "handoff-notes" },
        label: "/handoff-notes",
        description: "Provider handoff helper",
      },
      {
        id: "global-skill-command:handoff",
        type: "global-skill-command",
        skillCommandId: SkillCommandId.make("handoff"),
        prompt: "Summarize where we left off.",
        label: "/handoff",
        description: "Summarize where we left off.",
      },
    ] satisfies Array<
      Extract<ComposerCommandItem, { type: "global-skill-command" | "provider-slash-command" }>
    >;

    expect(searchSlashCommandItems(items, "handoff").map((item) => item.id)).toEqual([
      "global-skill-command:handoff",
      "provider-slash-command:claudeAgent:handoff-notes",
    ]);
  });
});
