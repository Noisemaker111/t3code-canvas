...
import { FixErrorIconButton } from "~/components/ui/fixErrorButton";
...
/** Default auto-dismiss for all toasts unless a call site sets `dismissAfterVisibleMs`. */
const DEFAULT_TOAST_DISMISS_MS = 5000;

function resolveToastDismissAfterVisibleMs(
  toast: { type?: unknown; data?: ThreadToastData | undefined },
  _hasActionableControls: boolean,
): number | undefined {
  if (toast.data?.dismissAfterVisibleMs !== undefined) {
    return toast.data.dismissAfterVisibleMs;
  }
  return DEFAULT_TOAST_DISMISS_MS;
}
...
/** Every error toast, regardless of call site, also lands in the persistent error log behind the badge. */
function useLogErrorToasts(
  ...
  useEffect(() => {
    const addEntry = useErrorLogStore.getState().addEntry;
    for (const toast of toasts) {
      if (toast.type !== "error") continue;
      addEntry({
        id: String(toast.id),
        title: typeof toast.title === "string" && toast.title ? toast.title : "Error",
        description:
...
interface ToastBodyDescriptor {
  ...
  /** The error, in error-log shape, when this toast is one Fix it can act on. */
  readonly fixError: ErrorLogEntry | null;
  readonly hasTrailingControls: boolean;
  readonly hasActionableControls: boolean;
...
function deriveToastBodyDescriptor(toast: {
  ...
  const copyErrorText =
    toast.type === "error" && typeof toast.description === "string" && !toast.data?.hideCopyButton
      ? toast.description
      : null;
  // Same id the error log stored this toast under, so a filed fix clears it there too.
  const fixError: ErrorLogEntry | null =
    toast.type === "error"
      ? {
          id: String(toast.id),
          title: typeof toast.title === "string" && toast.title ? toast.title : "Error",
  ...
  const hasActionableControls =
    toast.actionProps !== undefined || hasAdditionalActions || hasSecondaryAction;
  const hasTrailingControls = copyErrorText !== null || fixError !== null || hasActionableControls;
    ...
    <>
      ...
      {hasTrailingControls ? (
        ...
        >
          ...
          {fixError !== null ? <FixErrorIconButton errors={[fixError]} /> : null}
          ...
          return (
            ...
            >
              <ThreadToastVisibleAutoDismiss
                dismissAfterVisibleMs={resolveToastDismissAfterVisibleMs(
                  toast,
                  bodyDescriptor.hasActionableControls,
                )}
                toastId={toast.id}
              />
