...
import {
  ...
  expandLeadingComposerSkillCommand,
...
import {
  ...
  resolveComposerScrollToEndViewOffset,
...
import { deriveMessageMetrics, deriveTurnMetrics } from "../lib/contextWindow";
import { providerUsageUnusableReason } from "../lib/providerUsage";
import { getTerminalFocusOwner } from "../lib/terminalFocus";
import { resolveHostConsoleCwd } from "../lib/hostConsole";
import { resolveNewDraftStartFromOrigin } from "../lib/chatThreadActions";
import { consumeBoardDraftAutoSend } from "../lib/boardDraftSend";
import {
...
// How long a transient connection blip must persist before we surface the
// "reconnecting" banner. A mobile app-switch suspends the page and kills the
// socket; on return it re-establishes automatically in ~1-2s with the cached
// thread content staying on screen the whole time. Flashing a disruptive
// warning for that self-healing reconnect on every app-switch is exactly what
// makes it feel broken, so hold the banner back past a normal reconnect.
const RECONNECT_BANNER_GRACE_MS = 6_000;

// Debounce a boolean to `true` only after it has stayed `true` continuously for
// `delayMs`; it flips back to `false` immediately. Used so a quick, expected
// reconnect never shows any UI, while a genuinely stuck connection still does.
function useDelayedTrue(value: boolean, delayMs: number): boolean {
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => setDelayed(true), delayMs);
...
const PersistentThreadTerminalDrawer = memo(function PersistentThreadTerminalDrawer({
  ...
  const hostConsoleCwd = useAtomValue(serverEnvironment.configValueAtom(threadRef.environmentId));
  const cwd = useMemo(
    () =>
      launchContext?.cwd ??
      (project
        ...
        : resolveHostConsoleCwd(hostConsoleCwd)),
    [effectiveWorktreePath, hostConsoleCwd, launchContext?.cwd, project],
  );
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // A hard failure (blocked/auth) is worth showing right away; a transient
  // connecting/reconnecting/offline blip is debounced so the common mobile
  // app-switch reconnect stays invisible. Send-gating below still keys off
  // `activeEnvironmentUnavailable` directly, so this only affects the banner.
  const activeEnvironmentConnectionHardError = activeEnvironmentConnectionPhase === "error";
  const reconnectGraceElapsed = useDelayedTrue(
    activeEnvironmentUnavailable && !activeEnvironmentConnectionHardError,
    RECONNECT_BANNER_GRACE_MS,
  );
  const showEnvironmentUnavailableBanner =
    activeEnvironmentUnavailable && (activeEnvironmentConnectionHardError || reconnectGraceElapsed);
  ...
  const composerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
    ...
    if (activeEnvironmentUnavailableState && showEnvironmentUnavailableBanner) {
      ...
      items.push({
        ...
        description:
          connection.error ??
          "T3J is reconnecting automatically. Open Connections only if this persists.",
        actions: (
          <Button
            size="xs"
            variant="outline"
            onClick={() =>
              void navigate({ to: "/kanban", search: { station: "settings:connections" } })
            }
          >
            Connections
          </Button>
    ...
    showEnvironmentUnavailableBanner,
    navigate,
  ...
  const turnMetricsByTurnId = useMemo(
    () => deriveTurnMetrics(threadActivities),
    [threadActivities],
  );
  const messageMetricsByMessageId = useMemo(
    () => deriveMessageMetrics(threadActivities),
    [threadActivities],
  );
  ...
  const agentStatus = deriveAgentStatus({
    ...
    lastError: activeThread?.session?.lastError ?? null,
    isWorking,
    workLogEntries,
  });
  ...
  const scrollToEnd = useCallback(
    (animated = false) => {
      ...
      void legendListRef.current?.scrollToEnd?.({
        animated,
        viewOffset: resolveComposerScrollToEndViewOffset(composerOverlayHeight),
      });
    },
    [composerOverlayHeight],
  );
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    const usageBlock = providerUsageUnusableReason(
      usageSnapshot.data,
      String(ctxSelectedModelSelection.instanceId),
    );
    if (usageBlock) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Provider usage exhausted",
          description: `${usageBlock} Switch model or wait for the reset.`,
        }),
      );
      return;
    }
    ...
    // Expand a leading `/name` global skill command into its stored prompt
    // text here, right before the message is built for the timeline/wire —
    // not in the composer itself (which keeps showing the literal `/name`
    // token until send) and not in `trimmed`/`titleSeed` above (thread
    // auto-titling stays on the short literal text).
    const skillCommandPromptsById = new Map(
      Object.entries(settings.skillCommands).map(([id, command]) => [
        id.toLowerCase(),
        command.prompt,
      ]),
    );
    const expandedPromptForSend = expandLeadingComposerSkillCommand(
      promptForSend,
      skillCommandPromptsById,
    );
    ...
    const forcedBoardTitle = boardDraftSendOptionsRef.current?.forceTitle?.trim();
    boardDraftSendOptionsRef.current = null;
    let titleSeed = forcedBoardTitle || trimmed;
    if (!titleSeed) {
    ...
    const title = forcedBoardTitle || truncate(titleSeed);
  ...
  // Board drafts (Active launch / Hermes): prompt prefilled + one-shot auto-send.
  const boardDraftSendOptionsRef = useRef<{ forceTitle?: string } | null>(null);
  useEffect(() => {
    const sendKey =
      routeKind === "draft" && draftId
        ...
        : routeKind === "server" && routeThreadRef
          ...
          : null;
    if (!sendKey) return;
    const options = consumeBoardDraftAutoSend(sendKey);
    if (!options) return;
    boardDraftSendOptionsRef.current = options;
    const timer = window.setTimeout(() => {
      const target = routeKind === "draft" && draftId ? draftId : routeThreadRef;
      if (!target) return;
      const stored =
        useComposerDraftStore.getState().getComposerDraft(target)?.prompt?.trim() ?? "";
      if (!stored) return;
      promptRef.current = stored;
      void onSend();
    }, 350);
    return () => window.clearTimeout(timer);
    // Intentionally only re-run when the route identity changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- board one-shot launch
  }, [routeKind, draftId, routeThreadRef?.environmentId, routeThreadRef?.threadId]);
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      <BtwPanel threadRef={routeThreadRef} />
      ...
      >
        ...
        />
        <AgentStatusIndicator level={agentStatus.level} message={agentStatus.message ?? null} />
          ...
          <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
            ...
            <div className="relative flex min-h-0 flex-1 flex-col">
              ...
              <MessagesTimeline
                ...
                turnMetricsByTurnId={turnMetricsByTurnId}
                messageMetricsByMessageId={messageMetricsByMessageId}
                activeThreadEnvironmentId={activeThread.environmentId}
