...
const wakeupsLayer = Wakeups.layer({
  changes: Stream.merge(
    Stream.callback<"application-active">((queue) =>
      Effect.acquireRelease(
        Effect.sync(() => {
          const emitActive = () => Queue.offerUnsafe(queue, "application-active");
          // Track whether the page was actually backgrounded so the focus/pageshow
          // backups below only wake on a genuine resume, not on every in-page
          // focus change during normal use.
          ...
          // Mobile browsers do not reliably fire `visibilitychange` when the app
          // returns to the foreground. Swiping back to the app (or a back
          // navigation) frequently restores the page from the bfcache — a
          // `pageshow` with `persisted` — or only refocuses the window, with no
          // visibility transition at all. Without a wakeup on those paths the
          // connection supervisor never re-probes a socket that silently died
          // while the page was frozen, and the thread/shell subscriptions never
          // reload, so the UI is left on stale state — a "Working…" spinner that
          // never clears, model output that never resumes — until the user
          // manually acts. Waking on `pageshow`/`focus` (guarded so they only
          // fire after the page was hidden) closes that gap; the wakeup is
          // idempotent (a cheap liveness probe plus a resubscribe), so an extra
          // wake on a still-healthy connection is harmless.
          ...
          return { onVisibilityChange, onPageShow, onFocus };
        }),
        ({ onVisibilityChange, onPageShow, onFocus }) =>
          Effect.sync(() => {
