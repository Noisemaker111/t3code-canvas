 ...
 * - Mobile / coarse pointer: Enter inserts a newline (use Send button);
 *   Ctrl/Cmd+Enter still sends
 */
...
/**
 * Expand a leading `/name` global skill command token into its stored
 * prompt text at send time, preserving anything typed after it. The
 * composer itself never rewrites the box to show the expanded text (see
 * `onSelectComposerItem`'s `global-skill-command` branch) — this only runs
 * once, right before the message is added to the conversation and sent, so
 * both the timeline entry and the outgoing payload contain the expanded
 * prompt rather than the literal token. An unregistered or mistyped
 * command (e.g. after the skill was deleted) is a no-op: the message sends
 * as plain text, the same way a mistyped `/plan` does today.
 *
 * `skillCommandPrompts` is keyed by lowercased `SkillCommandId` — callers
 * should normalize lookups the same way the Skill Commands settings UI
 * normalizes ids on save.
 */
export function expandLeadingComposerSkillCommand(
  text: string,
  skillCommandPrompts: ReadonlyMap<string, string>,
): string {
  const match = /^\/([a-zA-Z][a-zA-Z0-9_-]*)(?=\s|$)/.exec(text);
  if (!match) {
    return text;
  }
  const prompt = skillCommandPrompts.get(match[1]!.toLowerCase());
  if (prompt === undefined) {
    return text;
  }
  return `${prompt}${text.slice(match[0].length)}`;
}
