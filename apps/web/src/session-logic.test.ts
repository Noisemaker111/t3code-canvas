...
import {
  ...
  workEntryIsProviderRetryWarning,
  type WorkLogEntry,
} from "./session-logic";
...
describe("deriveWorkLogEntries durations", () => {
  it("reports a tool call's full start→completion span", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-start",
        createdAt: "2026-02-23T00:00:00.000Z",
    ...
    const [entry] = deriveWorkLogEntries(activities);
    // 0s → 10s, measured from the filtered-out tool.started, not the first kept update.
    expect(entry?.durationMs).toBe(10_000);
  });

  it("falls back to the gap until the next step for spanless entries", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "thought",
        createdAt: "2026-02-23T00:00:00.000Z",
    ...
    const entries = deriveWorkLogEntries(activities);
    expect(entries[0]?.durationMs).toBe(5_000);
    // The trailing entry has no following step, so no duration is inferred.
    expect(entries[1]?.durationMs).toBeUndefined();
  });

  it("reports sub-second real spans", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-start",
        createdAt: "2026-02-23T00:00:00.000Z",
        kind: "tool.started",
        summary: "Bash",
        payload: { data: { toolCallId: "tc-2" } },
      }),
      makeActivity({
        id: "tool-complete",
        createdAt: "2026-02-23T00:00:00.400Z",
        kind: "tool.completed",
        summary: "Bash",
        payload: { data: { toolCallId: "tc-2" } },
      }),
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.durationMs).toBe(400);
  });

  it("prefers the server-stamped duration over derived spans", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-start",
        createdAt: "2026-02-23T00:00:00.000Z",
      ...
      makeActivity({
        ...
        kind: "tool.completed",
        summary: "Bash",
        payload: { itemId: "item-1", durationMs: 7_250, startedAt: "2026-02-23T00:00:01.750Z" },
      }),
    ];

    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.durationMs).toBe(7_250);
  });

  it("matches tool lifecycle spans by payload itemId", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-start",
        createdAt: "2026-02-23T00:00:00.000Z",
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.durationMs).toBe(6_000);
  });

  it("never attributes the gap until the next step to a tool row", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-complete",
        createdAt: "2026-02-23T00:00:00.000Z",
      ...
      makeActivity({
        ...
        kind: "tool.completed",
        summary: "Read",
        payload: { itemId: "item-4" },
      }),
    ];

    const entries = deriveWorkLogEntries(activities);
    // No tool.started to span from — better no duration than a made-up one.
    expect(entries[0]?.durationMs).toBeUndefined();
  });

  it("keeps server metrics through lifecycle collapse", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-updated",
        createdAt: "2026-02-23T00:00:02.000Z",
        kind: "tool.updated",
        summary: "Bash",
        payload: { itemId: "item-5", itemType: "command_execution" },
      }),
      makeActivity({
        id: "tool-complete",
        createdAt: "2026-02-23T00:00:10.000Z",
    ...
    const entries = deriveWorkLogEntries(activities);
    expect(entries).toHaveLength(1);
    expect(entries[0]?.tokenCount).toBe(321);
    expect(entries[0]?.durationMs).toBe(9_500);
  });
});

describe("deriveWorkLogEntries thoughts", () => {
  it("renders thought activities as thinking rows with tokens and time", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "thought-1",
        ...
        payload: {
          itemType: "reasoning",
          detail: "Weighing the options before editing.",
          itemId: "item-think",
          tokens: 42,
          durationMs: 5_500,
    ...
    expect(entry?.durationMs).toBe(5_500);
    expect(entry?.sourceActivityKind).toBe("thought");
    // Thought rows must stay visible: the timeline filters neutral tool rows.
    expect(workEntryIndicatesToolNeutralStatus(entry!)).toBe(false);
  });

  it("hides message.metrics bookkeeping activities from the work log", () => {
    const entries = deriveWorkLogEntries([
      makeActivity({
        id: "metrics-1",
        createdAt: "2026-02-23T00:00:08.000Z",
        ...
        tone: "info",
        payload: { messageId: "assistant:item-1", tokens: 128 },
      }),
    ]);
...
describe("workEntryIsProviderRetryWarning", () => {
  it("matches runtime.warning entries only", () => {
    expect(
      workEntryIsProviderRetryWarning(makeWorkEntry({ sourceActivityKind: "runtime.warning" })),
    ).toBe(true);
    expect(
      workEntryIsProviderRetryWarning(makeWorkEntry({ sourceActivityKind: "runtime.error" })),
    ).toBe(false);
    expect(workEntryIsProviderRetryWarning(makeWorkEntry({}))).toBe(false);
  });
});

describe("deriveAgentStatus", () => {
  it("reports error when the session failed", () => {
    expect(
      deriveAgentStatus({
        sessionStatus: "error",
        lastError: "Boom",
    ...
    ).toEqual({ level: "error", message: "Boom" });
  });

  it("reports error from lastError even without an error status", () => {
    expect(
      deriveAgentStatus({
  ...
  it("reports retrying while working and the latest entry is a retry warning", () => {
    const status = deriveAgentStatus({
      sessionStatus: "running",
      lastError: null,
      isWorking: true,
      workLogEntries: [
        makeWorkEntry({ id: "tool", sourceActivityKind: "tool.completed" }),
        makeWorkEntry({
          id: "warn",
          label: "rate-limited upstream",
          sourceActivityKind: "runtime.warning",
        }),
      ],
    });
    expect(status.level).toBe("retrying");
    expect(status.message).toBe("rate-limited upstream");
  });

  it("stays idle when the retry warning is not the latest entry", () => {
    expect(
      deriveAgentStatus({
        sessionStatus: "running",
        lastError: null,
        isWorking: true,
        workLogEntries: [
          makeWorkEntry({ id: "warn", sourceActivityKind: "runtime.warning" }),
          makeWorkEntry({ id: "tool", sourceActivityKind: "tool.completed" }),
        ],
      }).level,
    ).toBe("idle");
  ...
  it("stays idle when not working", () => {
    expect(
      deriveAgentStatus({
        sessionStatus: "idle",
        lastError: null,
        isWorking: false,
        workLogEntries: [makeWorkEntry({ sourceActivityKind: "runtime.warning" })],
      }).level,
    ).toBe("idle");
  });
});
