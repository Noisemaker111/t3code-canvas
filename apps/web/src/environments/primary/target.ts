...
/**
 * Windows (and some dual-stack hosts) can bind the server on `127.0.0.1` while
 * `localhost` resolves to `::1` first. Vite may serve the page on one loopback
 * name and leave `VITE_WS_URL` on another — HTTP often falls back, but the
 * WebSocket does not, so the board stays on "Loading…" forever. Align both
 * endpoints to the page hostname whenever they are all loopback.
 */
function alignLoopbackTargetToPage(target: PrimaryEnvironmentTarget): PrimaryEnvironmentTarget {
  ...
  try {
    pageHostname = normalizeHostname(
      parseTargetUrl({
        rawValue: pageHref,
  ...
  const align = (rawValue: string, urlKind: PrimaryEnvironmentUrlKind): string => {
    const url = parseTargetUrl({
      rawValue,
      source: target.source,
      urlKind,
    });
    if (!isLoopbackHostname(url.hostname) || normalizeHostname(url.hostname) === pageHostname) {
      return url.toString();
    }
  ...
  return {
    source: target.source,
    target: {
      httpBaseUrl: align(target.target.httpBaseUrl, "http-base-url"),
      wsBaseUrl: align(target.target.wsBaseUrl, "websocket-base-url"),
    },
  };
}
...
export function readPrimaryEnvironmentTarget(): PrimaryEnvironmentTarget {
  ...
  return alignLoopbackTargetToPage(resolved);
}
