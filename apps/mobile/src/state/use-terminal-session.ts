...
import { useTerminalOutputBuffer } from "./use-terminal-output";

/** A session plus its scrollback, which the native surface takes as a whole. */
export interface AttachedTerminalSession extends TerminalSessionState {
...
export function useAttachedTerminalSession(input: {
  ...
  const buffer = useTerminalOutputBuffer(
    input.terminal?.threadId ?? null,
    input.terminal?.terminalId ?? null,
  );
