...
describe("environment RPC", () => {
  ...
  it.effect("backs off doubling the retry delay while a subscription keeps failing", () =>
    Effect.gen(function* () {
      ...
      const settleAt = (expected: number) =>
        Effect.gen(function* () {
          for (let attempt = 0; attempt < 100; attempt += 1) {
            if ((yield* Ref.get(subscriptionCount)) >= expected) break;
            yield* Effect.yieldNow;
          }
          expect(yield* Ref.get(subscriptionCount)).toBe(expected);
        });
      ...
      yield* TestClock.adjust("100 millis");
      yield* settleAt(2);
      // …the second waits double, so the base delay alone must not resubscribe.
      yield* TestClock.adjust("100 millis");
      yield* settleAt(2);
      yield* TestClock.adjust("100 millis");
      yield* settleAt(3);
