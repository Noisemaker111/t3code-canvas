...
export type EnvironmentSubscriptionRpcTag =
  ...
  | typeof WS_METHODS.browserAttach
  | typeof WS_METHODS.subscribeBrowserSessions;

export type EnvironmentStreamCommandRpcTag =
  | typeof WS_METHODS.cloudInstallRelayClient
  | typeof WS_METHODS.gitRunStackedAction
  | typeof WS_METHODS.serverSubscribeInstallLog
  | typeof WS_METHODS.btwSendMessage;
...
interface SubscriptionOptions<TTag extends EnvironmentSubscriptionRpcTag> {
  ...
  /**
   * Base delay before resubscribing after an expected failure. Doubles on every
   * consecutive failure up to {@link RETRY_EXPECTED_FAILURE_CAP} — a subject
   * that keeps failing (a thread the server no longer knows, say) must not be
   * re-asked about several times a second forever.
   */
  readonly retryExpectedFailureAfter?: Duration.Input;
  readonly resubscribe?: Stream.Stream<unknown, never, never>;
}

const RETRY_EXPECTED_FAILURE_CAP = Duration.seconds(30);
/** Past this many doublings the delay is at any sane cap; stop shifting. */
const RETRY_EXPECTED_FAILURE_MAX_DOUBLINGS = 10;
            ...
            Option.match({
              ...
              onSome: (session) => {
                const method = session.client[tag] as (
                  ...
                  EnvironmentRpcStreamFailure<TTag>
                >;
                // Consecutive expected failures for this session; a delivered
                // value proves the subscription works again and resets it.
                            ...
                            Stream.catchCause((cause) => {
                              ...
                              if (
                                ...
                                const doublings = Math.min(
                                  expectedFailures,
                                  RETRY_EXPECTED_FAILURE_MAX_DOUBLINGS,
                                );
                                expectedFailures += 1;
