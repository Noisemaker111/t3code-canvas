  ...
  return {
    attach: createEnvironmentSubscriptionAtomFamily(runtime, {
      ...
      subscribe: (input: EnvironmentRpcInput<typeof WS_METHODS.terminalAttach>) => {
        const outputKey = terminalOutputKey(input);
        // Output leaves the stream here, before anything reactive sees it: the
        // surface reads it from its own mailbox, and this subscription only
        // ever emits when the *session* changes — status, error, exit.
        return subscribe(WS_METHODS.terminalAttach, input).pipe(
          Stream.tap((event) =>
            Effect.sync(() => {
              if (event.type === "output") publishTerminalOutput(outputKey, event.data);
              else if (event.type === "snapshot" || event.type === "restarted")
                resetTerminalOutput(outputKey, event.snapshot.history);
              else if (event.type === "cleared") resetTerminalOutput(outputKey, "");
            }),
          ),
          Stream.filter((event) => event.type !== "output" && event.type !== "activity"),
          Stream.scan(EMPTY_TERMINAL_BUFFER_STATE, applyTerminalAttachStreamEvent),
          Stream.ensuring(Effect.sync(() => forgetTerminalOutput(outputKey))),
        );
      },
    }),
