...
// A clock whose `advance` jumps the current time BEFORE waking due sleeps,
// the way a browser resuming a frozen page fires expired timers only after
// wall-clock time has already passed. TestClock cannot model this: it wakes
// each sleep with the clock set to that sleep's own deadline, so a woken
// fiber never observes a suspension gap.
function makeSuspendableClock() {
  let now = 0;
  const sleeps: Array<{ readonly at: number; readonly wake: Deferred.Deferred<void> }> = [];
  const clock: Clock.Clock = {
    ...
    sleep: (duration) =>
      Deferred.make<void>().pipe(
        Effect.flatMap((wake) => {
          sleeps.push({ at: now + Duration.toMillis(duration), wake });
  ...
  const advance = (ms: number) =>
    Effect.suspend(() => {
      now += ms;
      const due = sleeps.filter((entry) => entry.at <= now);
      for (const entry of due) {
        sleeps.splice(sleeps.indexOf(entry), 1);
      }
      return Effect.forEach(due, (entry) => Deferred.succeed(entry.wake, undefined), {
        discard: true,
      });
    });
...
describe("EnvironmentSupervisor", () => {
  ...
  it.effect("restarts a suspended connection attempt without surfacing a failure", () =>
    Effect.gen(function* () {
      const { clock, advance, settle } = makeSuspendableClock();
      const harness = yield* makeHarness({
        prepare: (attempt) => (attempt === 1 ? Effect.never : Effect.succeed(PREPARED_CONNECTION)),
      });
      const supervisor = yield* EnvironmentSupervisor.make(TARGET_ENTRY, {
        initiallyDesired: true,
      }).pipe(Effect.provide(harness.dependencies), Effect.provideService(Clock.Clock, clock));
      ...
      // A few foreground deadline slices elapse normally without exhausting
      ...
      // The page freezes mid-attempt; on resume the pending slice observes a
      // large wall-clock gap. The supervisor must restart the attempt silently
      // instead of reporting a bogus setup timeout.
      yield* advance(120_000);
      const connected = yield* awaitState(supervisor.state, (state) => state.phase === "connected");

      expect(connected.lastFailure).toBeNull();
  ...
  it.effect("keeps the session when a missed foreground probe immediately recovers", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const firstProbeFinished = yield* Deferred.make<void>();
      const harness = yield* makeHarness({
        probe: () =>
          Ref.updateAndGet(probeCount, (count) => count + 1).pipe(
            Effect.flatMap((count) =>
              count === 1
                ? Deferred.succeed(firstProbeFinished, undefined).pipe(
                    Effect.andThen(Effect.fail(transient("One probe was lost."))),
                  )
                : Effect.void,
            ),
          ),
      ...
      yield* awaitState(supervisor.state, (state) => state.phase === "connected");
      yield* harness.wake("application-active");
      yield* Deferred.await(firstProbeFinished);
      yield* TestClock.adjust("750 millis");
      for (let iteration = 0; iteration < 100 && (yield* Ref.get(probeCount)) < 2; iteration += 1) {
        yield* Effect.yieldNow;
      }
  ...
  it.effect("reconnects when the foreground liveness probe fails twice", () =>
    Effect.gen(function* () {
      const firstProbeFinished = yield* Deferred.make<void>();
      const harness = yield* makeHarness({
        probe: (attempt) =>
  ...
  it.effect("keeps a connected session warm with periodic keepalive probes", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const harness = yield* makeHarness({
      ...
      // The keepalive fires on its own cadence, with no application activation.
      yield* TestClock.adjust("30 seconds");
      for (let iteration = 0; iteration < 100 && (yield* Ref.get(probeCount)) < 1; iteration += 1) {
        yield* Effect.yieldNow;
      }
      expect(yield* Ref.get(probeCount)).toBe(1);

      // A healthy probe never reconnects: same session, no release, still connected.
      expect((yield* SubscriptionRef.get(supervisor.state)).phase).toBe("connected");
      expect(yield* Ref.get(harness.sessionCount)).toBe(1);
      expect(yield* Ref.get(harness.releaseCount)).toBe(0);

      // It keeps probing on every subsequent interval.
      yield* TestClock.adjust("30 seconds");
      for (let iteration = 0; iteration < 100 && (yield* Ref.get(probeCount)) < 2; iteration += 1) {
        yield* Effect.yieldNow;
      }
  ...
  it.effect("reconnects when two periodic keepalive probes fail", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        probe: (attempt) =>
          attempt === 1 ? Effect.fail(transient("The idle socket died.")) : Effect.void,
      });
      const supervisor = yield* EnvironmentSupervisor.make(TARGET_ENTRY, {
        initiallyDesired: true,
      }).pipe(Effect.provide(harness.dependencies));
      ...
      // One failed keepalive is confirmed once. Two failures reconnect rather
      // than leaving a dead-but-"connected" UI until the next user action.
      yield* TestClock.adjust("30 seconds");
      yield* TestClock.adjust("750 millis");
      yield* awaitState(supervisor.state, (state) => state.phase === "backoff");
      yield* TestClock.adjust("1 second");
