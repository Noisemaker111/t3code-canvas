...
export function connectionStatusText(connection: EnvironmentConnectionPresentation): string {
  switch (connection.phase) {
    ...
    case "connecting":
      return "Connecting...";
    case "reconnecting":
      // A dropped connection reconnecting is expected and self-healing (mobile
      // resume, a briefly reaped idle socket), so the status stays a calm
      // "Reconnecting..." rather than an alarming "Failed to connect." The
      // underlying reason is still surfaced separately (e.g. the banner
      // description) for anyone who needs to diagnose a persistent failure.
      return "Reconnecting...";
    case "connected":
      return "Connected";
    case "error":
