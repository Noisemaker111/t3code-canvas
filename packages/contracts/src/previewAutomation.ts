...
import { AgentBrowserProfileName } from "./agentBrowser.ts";
...
export const PreviewAutomationOpenInput = Schema.Struct({
  ...
  profile: Schema.optional(
    AgentBrowserProfileName.annotate({
      description:
        "Cookie jar to open the tab in. Jars keep their own logins and storage and run side by side, so 'staging' and 'prod', or two signed-in users, are two names. Defaults to 'default'; an unknown name is created.",
    }),
  ),
...
export class PreviewAutomationUnavailableError extends Schema.TaggedErrorClass<PreviewAutomationUnavailableError>()(
  ...
  {
    /** MCP toolkit capability the credential is missing (preview browser, kanban board, …). */
    capability: Schema.Literals(["preview", "kanban"]),
    environmentId: EnvironmentId,
    threadId: ThreadId,
