...
const decodeGlobalSkillCommand = Schema.decodeUnknownSync(GlobalSkillCommand);
...
describe("ServerSettings worktree defaults", () => {
  it("defaults new sessions to worktrees based on origin", () => {
...
describe("ServerSettings.skillCommands", () => {
  it("defaults to an empty record", () => {
    expect(DEFAULT_SERVER_SETTINGS.skillCommands).toEqual({});
    expect(decodeServerSettings({}).skillCommands).toEqual({});
  });

  it("decodes a valid entry, keyed by the command name", () => {
    const decoded = decodeServerSettings({
      skillCommands: {
        handoff: { prompt: "Summarize where we left off and what's next." },
      },
    });

    expect(decoded.skillCommands[SkillCommandId.make("handoff")]?.prompt).toBe(
      "Summarize where we left off and what's next.",
    );
  });

  it("rejects command names that violate the slug pattern", () => {
    expect(() =>
      decodeServerSettings({
        skillCommands: { "1bad": { prompt: "x" } },
      }),
    ).toThrow();
    expect(() =>
      decodeServerSettings({
        skillCommands: { "has space": { prompt: "x" } },
      }),
    ).toThrow();
  });

  it("rejects prompts over the max length", () => {
    expect(() => decodeGlobalSkillCommand({ prompt: "x".repeat(20_001) })).toThrow();
    expect(decodeGlobalSkillCommand({ prompt: "x".repeat(20_000) }).prompt).toHaveLength(20_000);
  });
});

describe("ServerSettingsPatch.skillCommands", () => {
  it("treats skillCommands as an optional whole-map replacement", () => {
    const patch = decodeServerSettingsPatch({});
    expect(patch.skillCommands).toBeUndefined();

    const replacement = decodeServerSettingsPatch({
      skillCommands: {
        handoff: { prompt: "Summarize where we left off." },
      },
    });
    expect(replacement.skillCommands?.[SkillCommandId.make("handoff")]?.prompt).toBe(
