...
export const ForgeIssueState = Schema.Literals(["open", "closed"]);
export type ForgeIssueState = typeof ForgeIssueState.Type;

/** One issue as the forge reports it. Labels are the forge's own tag vocabulary. */
export const ForgeIssue = Schema.Struct({
  provider: SourceControlProviderKind,
  number: PositiveInt,
  title: TrimmedNonEmptyString,
  url: Schema.String,
...
export type ForgeIssue = typeof ForgeIssue.Type;
...
/**
 * The commit identity of the account the server is signed in as.
 *
 * A forge picks the avatar for a commit from its author email, so the picture
 * cannot be set directly — `email` is the address that makes the forge resolve
 * the commit to `account`, and `avatarUrl` is the picture that resolution
 * yields.
 */
export const SourceControlAccountIdentity = Schema.Struct({
  provider: SourceControlProviderKind,
  account: TrimmedNonEmptyString,
  name: TrimmedNonEmptyString,
  email: TrimmedNonEmptyString,
  avatarUrl: Schema.Option(TrimmedNonEmptyString),
});
export type SourceControlAccountIdentity = typeof SourceControlAccountIdentity.Type;
