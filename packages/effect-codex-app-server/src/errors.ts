...
export class CodexAppServerProcessExitedError extends Schema.TaggedErrorClass<CodexAppServerProcessExitedError>()(
  "CodexAppServerProcessExitedError",
  {
    ...
    // Tail of the process's stderr, captured so an otherwise opaque non-zero
    // exit surfaces the actual reason (auth failure, config parse error, etc.)
    // rather than just a numeric code.
  ...
  override get message() {
    ...
    return this.stderr ? `${base}: ${this.stderr}` : base;
  }
}
