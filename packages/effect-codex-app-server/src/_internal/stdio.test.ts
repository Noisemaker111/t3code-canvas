...
import { makeChildStderrCapture, makeTerminationError } from "./stdio.ts";
...
describe("Codex App Server child process termination", () => {
  ...
  it.effect("surfaces the captured stderr tail in the exit message", () =>
    Effect.gen(function* () {
      const capture = makeChildStderrCapture();
      capture.append("error: not logged in\n");
      ...
      assert.equal(error.stderr, "error: not logged in");
      assert.equal(
        error.message,
        "Codex App Server process exited with code 1: error: not logged in",
      );
    }),
  );

  it.effect("omits stderr from the message when nothing was captured", () =>
    Effect.gen(function* () {
      const capture = makeChildStderrCapture();
