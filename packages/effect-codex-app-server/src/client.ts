...
import { makeChildStderrCapture, makeChildStdio, makeTerminationError } from "./_internal/stdio.ts";
...
const makeChildProcessClient = Effect.fn(
  ...
  // Keep draining stderr (so large diagnostics never block protocol responses)
  // but retain its tail so a non-zero exit reports the actual failure reason
  // instead of a bare exit code.
  const stderrCapture = makeChildStderrCapture();
  yield* Stream.runForEach(handle.stderr, (chunk) =>
    Effect.sync(() => stderrCapture.append(chunk)),
  ).pipe(Effect.ignore, Effect.forkScoped);
  return yield* make(
    makeChildStdio(handle),
    options,
    makeTerminationError(handle, stderrCapture.getTail),
  );
});
