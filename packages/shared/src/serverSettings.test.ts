...
describe("serverSettings helpers", () => {
  ...
  it("replaces the default model selection without leaking stale options", () => {
    const current = {
      ...DEFAULT_SERVER_SETTINGS,
  ...
  it("keeps the two model selections independent", () => {
    ...
    expect(next.defaultModelSelection).toEqual({
      instanceId: "claudeAgent",
      model: "claude-opus-5",
    });
  ...
  it("replaces the whole skillCommands map rather than merging entries", () => {
    const handoffId = SkillCommandId.make("handoff");
    const reviewId = SkillCommandId.make("review");
    const current = {
      ...DEFAULT_SERVER_SETTINGS,
      skillCommands: {
        [handoffId]: { prompt: "Old handoff prompt." },
  ...
  it("replaces boardSettings as a whole object", () => {
    const current = {
      ...DEFAULT_SERVER_SETTINGS,
      boardSettings: {
        ...DEFAULT_SERVER_SETTINGS.boardSettings,
